...
namespace physica::examples::smoke::simulation_cuda {
    namespace {
        ...
        __global__ void write_control_kernel(const Grid grid, const std::uint64_t step, const float pulse_period, const Vector left_center, const Vector right_center, const float source_radius, const float density_source_rate, const float temperature_source_rate, const Vector left_acceleration, const Vector right_acceleration, float* const density_source, float* const temperature_source, float* const acceleration_x, float* const acceleration_y, float* const acceleration_z) {
            ...
            const float left_weight = (0.75F + 0.25F * sinf(phase)) * gaussian(x, y, z, left_center, source_radius);
            const float right_weight = (0.75F + 0.25F * sinf(phase + 0.5F * two_pi)) * gaussian(x, y, z, right_center, source_radius);
    ...
    void write_control(const ::cuda::stream_ref stream, const Grid grid, const std::uint64_t step, const float pulse_period, const Vector left_center, const Vector right_center, const float source_radius, const float density_source_rate, const float temperature_source_rate, const Vector left_acceleration, const Vector right_acceleration, float* const density_source, float* const temperature_source, float* const acceleration_x, float* const acceleration_y, float* const acceleration_z) {
        const std::uint32_t cell_count = grid.nx * grid.ny * grid.nz;
        write_control_kernel<<<(cell_count + block_size - 1u) / block_size, block_size, 0u, stream.get()>>>(grid, step, pulse_period, left_center, right_center, source_radius, density_source_rate, temperature_source_rate, left_acceleration, right_acceleration, density_source, temperature_source, acceleration_x, acceleration_y, acceleration_z);
    }
} // namespace physica::examples::smoke::simulation_cuda
