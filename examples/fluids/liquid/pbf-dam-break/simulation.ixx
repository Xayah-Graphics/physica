...
export namespace physica::examples::pbf_dam_break {
    ...
    Simulation::Simulation() : stream{::cuda::devices[0]}, configuration(create_configuration()), solver(configuration, {.pressure_iterations = 5u, .checkpoint_interval = 2u}, fluids::liquid::particle::ExecutionMode::forward, stream), current_state(solver.allocate_state()), step_cache(solver.allocate_step_cache()), next_state(solver.allocate_state()), control(solver.allocate_control()), parameters(solver.allocate_parameters()) {
