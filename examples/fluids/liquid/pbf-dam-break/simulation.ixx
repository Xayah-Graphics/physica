...
export namespace physica::examples::pbf_dam_break {
    struct Simulation final {
        ...
        ::cuda::stream stream;
        const fluids::liquid::meshfree::Configuration configuration;
    ...
    Simulation::Simulation() : stream{::cuda::devices[0]}, configuration(create_configuration()), model(configuration, stream), solver({.pressure_iterations = 5u, .checkpoint_interval = 2u}), current_state(solver.allocate_state(model)), step_cache(solver.allocate_step_cache(model)), next_state(solver.allocate_state(model)), control(solver.allocate_control(model)), parameters(solver.allocate_parameters(model)), workspace(solver.allocate_workspace(model)) {
    ...
    void Simulation::reset() {
        ...
        solver.copy_state(model, current_state, next_state);
        stream.sync();
        step_index    = 0u;
        physical_time = 0.0;
    }
    ...
    fluids::liquid::meshfree::Configuration Simulation::create_configuration() {
        return {
