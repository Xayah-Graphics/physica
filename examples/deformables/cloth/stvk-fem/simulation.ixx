...
export module physica.example.deformables.cloth.stvk_fem;

import std;
import physica.deformables.cloth.model;
import physica.deformables.cloth.solvers.stvk_fem;
import physica.example.deformables.cloth.support.scene;

export namespace physica::examples::cloth::stvk_fem {
    struct Summary final {
        std::uint32_t frames;
        double physical_time;
        float maximum_absolute_green_strain;
    ...
    private:
        ...
        deformables::cloth::solvers::stvk_fem::Solver solver;
    ...
    Summary Simulation::summarize() {
        ...
        std::array<std::vector<float>, 3uz> green_strain{
        ...
        ::cuda::copy_bytes(stream, step_cache.green_strains.x, ::cuda::std::span<float>{green_strain[0].data(), triangle_count});
        ::cuda::copy_bytes(stream, step_cache.green_strains.y, ::cuda::std::span<float>{green_strain[1].data(), triangle_count});
        ::cuda::copy_bytes(stream, step_cache.green_strains.z, ::cuda::std::span<float>{green_strain[2].data(), triangle_count});
        ...
        float maximum_absolute_green_strain = 0.0F;
        double total_elastic_energy = 0.0;
        for (std::size_t triangle = 0uz; triangle < triangle_count; ++triangle) {
            const float difference = green_strain[0][triangle] - green_strain[2][triangle];
            const float radius = std::sqrt(0.25F * difference * difference + green_strain[1][triangle] * green_strain[1][triangle]);
            const float center = 0.5F * (green_strain[0][triangle] + green_strain[2][triangle]);
            maximum_absolute_green_strain = std::max(maximum_absolute_green_strain, std::max(std::abs(center + radius), std::abs(center - radius)));
        ...
        return {
            ...
            .maximum_absolute_green_strain = maximum_absolute_green_strain,
...
} // namespace physica::examples::cloth::stvk_fem
