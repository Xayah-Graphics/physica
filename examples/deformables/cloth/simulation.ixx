...
export namespace physica::examples::cloth {
    ...
    private:
        deformables::cloth::Solver<deformables::cloth::MassSpringForces, deformables::cloth::SemiImplicitEuler, deformables::cloth::FixedPositionConstraints> solver;
    ...
    private:
        ...
        deformables::cloth::Solver<deformables::cloth::MassSpringForces, deformables::cloth::SemiImplicitEuler, deformables::cloth::FixedPositionConstraints>::Parameters parameters;
        deformables::cloth::Solver<deformables::cloth::MassSpringForces, deformables::cloth::SemiImplicitEuler, deformables::cloth::FixedPositionConstraints>::StepCache step_cache;

        [[nodiscard]] static deformables::cloth::DomainConfiguration create_domain_configuration();
    };

    Simulation::Simulation() : stream{::cuda::devices[0]}, configuration(create_domain_configuration()), solver(configuration, {.gravity = {.x = 0.0F, .y = gravity_y, .z = 0.0F}}, {.time_step = time_step / static_cast<float>(integration_substeps)}, deformables::cloth::ExecutionMode::forward, stream), current_state(solver.allocate_state()), next_state(solver.allocate_state()), control(solver.allocate_control()), parameters(solver.allocate_parameters()), step_cache(solver.allocate_step_cache()) {
    ...
    void Simulation::step() {
        ...
        for (std::uint32_t substep = 0u; substep < integration_substeps; ++substep) {
            simulation_cuda::write_control(stream, {.rows = rows, .columns = columns, .width = width, .height = height}, step_index * integration_substeps + substep, substep_time_step, {.speed = wind_speed, .gust_strength = gust_strength, .gust_frequency = gust_frequency, .air_density = air_density, .drag_coefficient = drag_coefficient, .skin_drag_coefficient = skin_drag_coefficient, .ramp_duration = wind_ramp_duration}, current_state.positions.x.values.data(), current_state.positions.y.values.data(), current_state.positions.z.values.data(), current_state.velocities.x.values.data(), current_state.velocities.y.values.data(), current_state.velocities.z.values.data(), control.external_forces.x.values.data(), control.external_forces.y.values.data(), control.external_forces.z.values.data());
