...
export module physica.example.deformables.cloth.xpbd;

import std;
import physica.deformables.cloth.model;
import physica.deformables.cloth.solvers.xpbd;
import physica.example.deformables.cloth.support.scene;

export namespace physica::examples::cloth::xpbd {
    struct Summary final {
        std::uint32_t frames;
        double physical_time;
        float maximum_stretch_ratio;
        float maximum_fixed_position_error;
        float final_maximum_absolute_lambda;
    ...
    struct Simulation final {
        ...
        inline static constexpr std::uint32_t projection_iterations = 32u;
        inline static constexpr float gravity_y                     = -9.81F;
        inline static constexpr float mass                          = 0.04F;
        inline static constexpr float stretch_compliance            = 1.0e-5F;
        inline static constexpr float bending_compliance            = 5.0e-4F;
    ...
    private:
        ...
        deformables::cloth::solvers::xpbd::Solver solver;
    ...
    Summary Simulation::summarize() {
        ...
        std::vector<float> lambdas(step_cache.lambdas.values.size());
        Vector3<float> probe_velocity{};
        ...
        ::cuda::copy_bytes(stream, step_cache.lambdas.values, ::cuda::std::span<float>{lambdas.data(), lambdas.size()});
        stream.sync();
        ...
        float final_maximum_absolute_lambda = 0.0F;
        for (const float lambda : lambdas) final_maximum_absolute_lambda = std::max(final_maximum_absolute_lambda, std::abs(lambda));
        ...
        return {
            ...
            .final_maximum_absolute_lambda = final_maximum_absolute_lambda,
...
} // namespace physica::examples::cloth::xpbd
