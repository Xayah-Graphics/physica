...
Sampling<Shape, NetworkSpec>::Sampling(const ::cuda::stream_ref source_stream)
    : stream{source_stream},
      samples{stream, ::cuda::device_default_memory_pool(stream.device()), NetworkSpec.inference_capacity, ::cuda::no_init},
      rays{stream, ::cuda::device_default_memory_pool(stream.device()), NetworkSpec.training_batch_size, ::cuda::no_init},
      target_pixel_indices{stream, ::cuda::device_default_memory_pool(stream.device()), NetworkSpec.training_batch_size, ::cuda::no_init},
      numsteps{stream, ::cuda::device_default_memory_pool(stream.device()), static_cast<std::size_t>(NetworkSpec.training_batch_size) * 2uz, ::cuda::no_init},
      ray_counter{stream, ::cuda::device_default_memory_pool(stream.device()), 1uz, ::cuda::no_init},
      sample_counter{stream, ::cuda::device_default_memory_pool(stream.device()), 1uz, ::cuda::no_init},
      occupancy{stream, ::cuda::device_default_memory_pool(stream.device()), grid_cell_count / 8u, ::cuda::no_init},
      density{stream, ::cuda::device_default_memory_pool(stream.device()), grid_cell_count, ::cuda::no_init},
      density_scratch{stream, ::cuda::device_default_memory_pool(stream.device()), grid_cell_count, ::cuda::no_init},
      density_indices{stream, ::cuda::device_default_memory_pool(stream.device()), grid_cell_count, ::cuda::no_init},
      density_mean{stream, ::cuda::device_default_memory_pool(stream.device()), 1uz, ::cuda::no_init},
      occupied_cell_counter{stream, ::cuda::device_default_memory_pool(stream.device()), 1uz, ::cuda::no_init},
      evaluation_numsteps{stream, ::cuda::device_default_memory_pool(stream.device()), static_cast<std::size_t>(Shape.evaluation_tile_rays) * 2uz, ::cuda::no_init},
      evaluation_sample_counter{stream, ::cuda::device_default_memory_pool(stream.device()), 1uz, ::cuda::no_init},
      occupied_cell_count{stream, ::cuda::pinned_default_memory_pool(), 1uz, ::cuda::no_init} {
...
std::uint32_t Sampling<Shape, NetworkSpec>::adapt(const std::uint32_t generated_sample_count, const std::uint32_t requested_compacted_sample_count) {
    inference_sample_count = (((std::min)(generated_sample_count, NetworkSpec.inference_capacity) + Shape.training_batch_granularity - 1u) / Shape.training_batch_granularity) * Shape.training_batch_granularity;
    ...
    rays_per_batch = std::clamp(((target_rays + Shape.training_batch_granularity - 1u) / Shape.training_batch_granularity) * Shape.training_batch_granularity, Shape.training_batch_granularity, NetworkSpec.training_batch_size);
