...
namespace physica::reconstruction::instant_ngp::cuda_detail {
    ...
    __global__ void initialize_mlp_params_kernel(const std::uint32_t seed, float* __restrict__ params_full_precision, __half* __restrict__ params, __half* __restrict__ param_gradients) {
        ...
        float scale = 0.0f;
        if (i < NetworkLayout<network_cuda_shape>::network_parameter_layout.density_output_weight_offset) scale = sqrtf(6.0f / static_cast<float>(NetworkLayout<network_cuda_shape>::mlp_width + NetworkLayout<network_cuda_shape>::grid_output_width));
        else if (i < NetworkLayout<network_cuda_shape>::network_parameter_layout.rgb_input_weight_offset) scale = sqrtf(6.0f / static_cast<float>(NetworkLayout<network_cuda_shape>::density_output_width + NetworkLayout<network_cuda_shape>::mlp_width));
        else if (i < NetworkLayout<network_cuda_shape>::network_parameter_layout.rgb_hidden_weight_offset) scale = sqrtf(6.0f / static_cast<float>(NetworkLayout<network_cuda_shape>::mlp_width + NetworkLayout<network_cuda_shape>::rgb_input_width));
        else if (i < NetworkLayout<network_cuda_shape>::network_parameter_layout.rgb_output_weight_offset) scale = sqrtf(6.0f / static_cast<float>(NetworkLayout<network_cuda_shape>::mlp_width + NetworkLayout<network_cuda_shape>::mlp_width));
        else scale = sqrtf(6.0f / static_cast<float>(NetworkLayout<network_cuda_shape>::network_output_width + NetworkLayout<network_cuda_shape>::mlp_width));
