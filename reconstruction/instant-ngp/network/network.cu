...
struct NetworkLayout final {
    ...
    static consteval ParameterLayout make_parameter_layout() {
        ...
        constexpr std::uint32_t grid_hashmap_size = 1u << Shape.grid_log2_hashmap_size;
        ...
        for (std::uint32_t level = 0u; level < Shape.grid_level_count; ++level) {
            ...
            std::uint64_t positions = dense < grid_max_positions ? dense : grid_max_positions;
            positions = ((positions + 7u) / 8u) * 8u;
            positions = positions < grid_hashmap_size ? positions : grid_hashmap_size;
...
__global__ void initialize_mlp_params_kernel(const std::uint32_t seed, float* __restrict__ params_full_precision, __half* __restrict__ params, __half* __restrict__ param_gradients) {
    ...
    float scale = 0.0f;
    if (i < NetworkLayout<network_cuda_shape>::network_parameter_layout.density_output_weight_offset)
        scale = sqrtf(6.0f / static_cast<float>(NetworkLayout<network_cuda_shape>::mlp_width + NetworkLayout<network_cuda_shape>::grid_output_width));
    else if (i < NetworkLayout<network_cuda_shape>::network_parameter_layout.rgb_input_weight_offset)
        scale = sqrtf(6.0f / static_cast<float>(NetworkLayout<network_cuda_shape>::density_output_width + NetworkLayout<network_cuda_shape>::mlp_width));
    else if (i < NetworkLayout<network_cuda_shape>::network_parameter_layout.rgb_hidden_weight_offset)
        scale = sqrtf(6.0f / static_cast<float>(NetworkLayout<network_cuda_shape>::mlp_width + NetworkLayout<network_cuda_shape>::rgb_input_width));
    else if (i < NetworkLayout<network_cuda_shape>::network_parameter_layout.rgb_output_weight_offset)
        scale = sqrtf(6.0f / static_cast<float>(NetworkLayout<network_cuda_shape>::mlp_width + NetworkLayout<network_cuda_shape>::mlp_width));
    else
...
__global__ void mlp_forward_64_relu_kernel(const std::uint32_t batch_size, const __half* __restrict__ input, const __half* __restrict__ weights, __half* __restrict__ hidden, __half* __restrict__ output) {
    ...
    if constexpr (OutputRowMajor)
        mlp_last_layer_forward(shmem, last_weights, output + elem_idx * NetworkLayout<network_cuda_shape>::network_output_width, NetworkLayout<network_cuda_shape>::network_output_width, nvcuda::wmma::mem_row_major);
    else
...
__global__ void adam_step_kernel(float* __restrict__ params_full_precision, __half* __restrict__ params, const __half* __restrict__ gradients, float* __restrict__ first_moments, float* __restrict__ second_moments, std::uint32_t* __restrict__ param_steps) {
    ...
    const float corrected_lr = 1e-2F * sqrtf(1.0f - powf(0.99F, static_cast<float>(step))) / (1.0f - powf(0.9F, static_cast<float>(step)));
    const float updated_param = param - corrected_lr * first_moment / (sqrtf(second_moment) + 1e-15F);
...
void NetworkKernels<Shape>::initialize_cublaslt_matmul_plans(const cublasLtHandle_t cublaslt, cublasLtMatmulDesc_t* const operation_descriptors, cublasLtMatrixLayout_t* const a_descriptors, cublasLtMatrixLayout_t* const b_descriptors, cublasLtMatrixLayout_t* const output_descriptors, cublasLtMatmulAlgo_t* const algorithms) {
    ...
    cuda_detail::initialize_cublaslt_matmul_plan(cublaslt, "rgb last weight gradients", operation_descriptors[0], a_descriptors[0], b_descriptors[0], output_descriptors[0], algorithms[0], {CUBLASLT_ORDER_COL, NetworkLayout<Shape>::network_output_width, batch, NetworkLayout<Shape>::network_output_width}, {CUBLASLT_ORDER_ROW, batch, NetworkLayout<Shape>::mlp_width, NetworkLayout<Shape>::mlp_width}, {CUBLASLT_ORDER_ROW, NetworkLayout<Shape>::network_output_width, NetworkLayout<Shape>::mlp_width, NetworkLayout<Shape>::mlp_width});
    cuda_detail::initialize_cublaslt_matmul_plan(cublaslt, "rgb hidden weight gradients", operation_descriptors[1], a_descriptors[1], b_descriptors[1], output_descriptors[1], algorithms[1], {CUBLASLT_ORDER_COL, NetworkLayout<Shape>::mlp_width, batch, NetworkLayout<Shape>::mlp_width}, {CUBLASLT_ORDER_ROW, batch, NetworkLayout<Shape>::mlp_width, NetworkLayout<Shape>::mlp_width}, {CUBLASLT_ORDER_ROW, NetworkLayout<Shape>::mlp_width, NetworkLayout<Shape>::mlp_width, NetworkLayout<Shape>::mlp_width});
    cuda_detail::initialize_cublaslt_matmul_plan(cublaslt, "rgb first weight gradients", operation_descriptors[2], a_descriptors[2], b_descriptors[2], output_descriptors[2], algorithms[2], {CUBLASLT_ORDER_COL, NetworkLayout<Shape>::mlp_width, batch, NetworkLayout<Shape>::mlp_width}, {CUBLASLT_ORDER_COL, batch, NetworkLayout<Shape>::rgb_input_width, batch}, {CUBLASLT_ORDER_ROW, NetworkLayout<Shape>::mlp_width, NetworkLayout<Shape>::rgb_input_width, NetworkLayout<Shape>::rgb_input_width});
    cuda_detail::initialize_cublaslt_matmul_plan(cublaslt, "rgb input gradients", operation_descriptors[3], a_descriptors[3], b_descriptors[3], output_descriptors[3], algorithms[3], {CUBLASLT_ORDER_COL, NetworkLayout<Shape>::rgb_input_width, NetworkLayout<Shape>::mlp_width, NetworkLayout<Shape>::rgb_input_width}, {CUBLASLT_ORDER_COL, NetworkLayout<Shape>::mlp_width, batch, NetworkLayout<Shape>::mlp_width}, {CUBLASLT_ORDER_ROW, NetworkLayout<Shape>::rgb_input_width, batch, batch});
    cuda_detail::initialize_cublaslt_matmul_plan(cublaslt, "density last weight gradients", operation_descriptors[4], a_descriptors[4], b_descriptors[4], output_descriptors[4], algorithms[4], {CUBLASLT_ORDER_ROW, NetworkLayout<Shape>::density_output_width, batch, batch}, {CUBLASLT_ORDER_ROW, batch, NetworkLayout<Shape>::mlp_width, NetworkLayout<Shape>::mlp_width}, {CUBLASLT_ORDER_ROW, NetworkLayout<Shape>::density_output_width, NetworkLayout<Shape>::mlp_width, NetworkLayout<Shape>::mlp_width});
...
void NetworkKernels<Shape>::initialize_mlp_parameters(const ::cuda::stream_ref stream, const std::uint32_t seed, float* const params_full_precision, std::uint16_t* const params, std::uint16_t* const param_gradients) {
    constexpr std::uint32_t blocks = (NetworkLayout<Shape>::network_parameter_layout.mlp_param_count + 128u - 1u) / 128u;
    ::cuda::launch(stream, ::cuda::make_config(::cuda::make_hierarchy(::cuda::grid_dims(blocks), ::cuda::block_dims<128u>())), cuda_detail::initialize_mlp_params_kernel, seed, params_full_precision, reinterpret_cast<__half*>(params), reinterpret_cast<__half*>(param_gradients));
}
...
void NetworkKernels<Shape>::upload_trainable_parameters(const ::cuda::stream_ref stream, const float* const params_full_precision, float* const out_params_full_precision, std::uint16_t* const out_params, std::uint16_t* const out_param_gradients) {
    ...
    constexpr std::uint32_t blocks = (NetworkLayout<Shape>::network_parameter_layout.total_param_count + 128u - 1u) / 128u;
    ::cuda::launch(stream, ::cuda::make_config(::cuda::make_hierarchy(::cuda::grid_dims(blocks), ::cuda::block_dims<128u>())), cuda_detail::cast_params_to_half_kernel<NetworkLayout<Shape>::network_parameter_layout.total_param_count>, out_params_full_precision, reinterpret_cast<__half*>(out_params));
}
...
void NetworkKernels<Shape>::evaluate_network(const ::cuda::stream_ref stream, const std::uint32_t sample_count, const float* const sample_coords, const std::uint16_t* const params, std::uint16_t* const density_input, std::uint16_t* const rgb_input, std::uint16_t* const network_output) {
    ...
    for (std::uint32_t offset = 0u; offset < sample_count; offset += NetworkLayout<Shape>::network_batch_size) {
        ...
        const dim3 grid_blocks{(chunk + 512u - 1u) / 512u, NetworkLayout<Shape>::grid_n_levels, 1u};
        ::cuda::launch(stream, ::cuda::make_config(::cuda::make_hierarchy(::cuda::grid_dims(grid_blocks), ::cuda::block_dims<512u>())), cuda_detail::encode_grid_forward_kernel, chunk, NetworkLayout<Shape>::network_parameter_layout.grid_offsets, chunk_sample_coords, reinterpret_cast<const __half*>(params + NetworkLayout<Shape>::network_parameter_layout.grid_param_offset), reinterpret_cast<__half*>(density_input));

        const auto linear_config = ::cuda::distribute<128u>(chunk);
        ::cuda::launch(stream, linear_config, cuda_detail::encode_spherical_harmonics_kernel, chunk, chunk_sample_coords, reinterpret_cast<__half*>(rgb_input) + static_cast<std::uint64_t>(NetworkLayout<Shape>::density_output_width) * chunk);
        ...
        ::cuda::launch(stream, mlp_config, cuda_detail::mlp_forward_64_relu_kernel<false, NetworkLayout<Shape>::density_hidden_layers>, chunk, reinterpret_cast<const __half*>(density_input), reinterpret_cast<const __half*>(params + NetworkLayout<Shape>::network_parameter_layout.density_param_offset), nullptr, reinterpret_cast<__half*>(rgb_input));

        ::cuda::launch(stream, mlp_config, cuda_detail::mlp_forward_64_relu_kernel<true, NetworkLayout<Shape>::rgb_hidden_layers>, chunk, reinterpret_cast<const __half*>(rgb_input), reinterpret_cast<const __half*>(params + NetworkLayout<Shape>::network_parameter_layout.rgb_param_offset), nullptr, reinterpret_cast<__half*>(network_output) + static_cast<std::uint64_t>(offset) * NetworkLayout<Shape>::network_output_width);

        ::cuda::launch(stream, linear_config, cuda_detail::extract_density_kernel, chunk, reinterpret_cast<const __half*>(rgb_input), reinterpret_cast<__half*>(network_output) + static_cast<std::uint64_t>(offset) * NetworkLayout<Shape>::network_output_width);
    }
}
...
void NetworkKernels<Shape>::forward_network(const ::cuda::stream_ref stream, const float* const sample_coords, const std::uint16_t* const params, std::uint16_t* const density_input, std::uint16_t* const rgb_input, std::uint16_t* const density_forward_hidden, std::uint16_t* const rgb_forward_hidden, std::uint16_t* const network_output) {
    ...
    constexpr dim3 grid_blocks{(NetworkLayout<Shape>::network_batch_size + 512u - 1u) / 512u, NetworkLayout<Shape>::grid_n_levels, 1u};
    ::cuda::launch(stream, ::cuda::make_config(::cuda::make_hierarchy(::cuda::grid_dims(grid_blocks), ::cuda::block_dims<512u>())), cuda_detail::encode_grid_forward_kernel, NetworkLayout<Shape>::network_batch_size, NetworkLayout<Shape>::network_parameter_layout.grid_offsets, sample_coords, reinterpret_cast<const __half*>(params + NetworkLayout<Shape>::network_parameter_layout.grid_param_offset), reinterpret_cast<__half*>(density_input));

    constexpr auto linear_config = ::cuda::distribute<128u>(NetworkLayout<Shape>::network_batch_size);
    ::cuda::launch(stream, linear_config, cuda_detail::encode_spherical_harmonics_kernel, NetworkLayout<Shape>::network_batch_size, sample_coords, reinterpret_cast<__half*>(rgb_input) + static_cast<std::uint64_t>(NetworkLayout<Shape>::density_output_width) * NetworkLayout<Shape>::network_batch_size);

    constexpr int forward_shmem = sizeof(__half) * (16u + 16u * 8u) * (NetworkLayout<Shape>::mlp_width + 8u);
    constexpr dim3 threads{32u, NetworkLayout<Shape>::mlp_width_blocks, 1u};
    constexpr dim3 blocks{NetworkLayout<Shape>::network_batch_size / (16u * 8u), 1u, 1u};
    const auto mlp_config = ::cuda::make_config(::cuda::make_hierarchy(::cuda::grid_dims(blocks), ::cuda::block_dims(threads)), ::cuda::dynamic_shared_memory<std::uint8_t[]>(forward_shmem));
    ::cuda::launch(stream, mlp_config, cuda_detail::mlp_forward_64_relu_kernel<false, NetworkLayout<Shape>::density_hidden_layers>, NetworkLayout<Shape>::network_batch_size, reinterpret_cast<const __half*>(density_input), reinterpret_cast<const __half*>(params + NetworkLayout<Shape>::network_parameter_layout.density_param_offset), reinterpret_cast<__half*>(density_forward_hidden), reinterpret_cast<__half*>(rgb_input));
    ...
    ::cuda::launch(stream, linear_config, cuda_detail::extract_density_kernel, NetworkLayout<Shape>::network_batch_size, reinterpret_cast<const __half*>(rgb_input), reinterpret_cast<__half*>(network_output));
}
...
void NetworkKernels<Shape>::backward_network(const ::cuda::stream_ref stream, const float* const sample_coords, const std::uint16_t* const params, std::uint16_t* const gradients, const std::uint16_t* const density_input, const std::uint16_t* const rgb_input, const std::uint16_t* const density_forward_hidden, const std::uint16_t* const rgb_forward_hidden, const std::uint16_t* const network_output, const std::uint16_t* const network_output_gradients, std::uint16_t* const rgb_output_gradients, std::uint16_t* const rgb_input_gradients, std::uint16_t* const density_input_gradients, std::uint16_t* const density_backward_hidden, std::uint16_t* const rgb_backward_hidden, const cublasLtHandle_t cublaslt, const cublasLtMatmulDesc_t* const operation_descriptors, const cublasLtMatrixLayout_t* const a_descriptors, const cublasLtMatrixLayout_t* const b_descriptors, const cublasLtMatrixLayout_t* const output_descriptors, const cublasLtMatmulAlgo_t* const algorithms, std::uint8_t* const cublaslt_workspace) {
    ...
    ::cuda::launch(stream, linear_config, cuda_detail::extract_rgb_gradients_kernel, NetworkLayout<Shape>::network_batch_size, reinterpret_cast<const __half*>(network_output_gradients), reinterpret_cast<__half*>(rgb_output_gradients));
    ...
    ::cuda::launch(stream, backward_config, cuda_detail::mlp_backward_hidden_64_relu_kernel<nvcuda::wmma::row_major, NetworkLayout<Shape>::rgb_hidden_layers>, NetworkLayout<Shape>::network_batch_size, reinterpret_cast<const __half*>(rgb_output_gradients), reinterpret_cast<const __half*>(params + NetworkLayout<Shape>::network_parameter_layout.rgb_param_offset), reinterpret_cast<const __half*>(rgb_forward_hidden), reinterpret_cast<__half*>(rgb_backward_hidden), NetworkLayout<Shape>::network_output_width);

    cuda_detail::cublaslt_matmul(
        stream,
        cublaslt,
        "rgb last weight gradients",
        operation_descriptors[0],
        a_descriptors[0],
        b_descriptors[0],
        output_descriptors[0],
        algorithms[0],
        reinterpret_cast<const __half*>(rgb_output_gradients),
        reinterpret_cast<const __half*>(rgb_forward_hidden) + (NetworkLayout<Shape>::rgb_hidden_layers - 1u) * hidden_layer_stride,
        reinterpret_cast<__half*>(gradients + NetworkLayout<Shape>::network_parameter_layout.rgb_output_weight_offset),
        cublaslt_workspace);

    cuda_detail::cublaslt_matmul(
        stream,
        cublaslt,
        "rgb hidden weight gradients",
        operation_descriptors[1],
        a_descriptors[1],
        b_descriptors[1],
        output_descriptors[1],
        algorithms[1],
        reinterpret_cast<const __half*>(rgb_backward_hidden),
        reinterpret_cast<const __half*>(rgb_forward_hidden),
        reinterpret_cast<__half*>(gradients + NetworkLayout<Shape>::network_parameter_layout.rgb_hidden_weight_offset),
        cublaslt_workspace);

    cuda_detail::cublaslt_matmul(
        stream,
        cublaslt,
        "rgb first weight gradients",
        operation_descriptors[2],
        a_descriptors[2],
        b_descriptors[2],
        output_descriptors[2],
        algorithms[2],
        reinterpret_cast<const __half*>(rgb_backward_hidden) + (NetworkLayout<Shape>::rgb_hidden_layers - 1u) * hidden_layer_stride,
        reinterpret_cast<const __half*>(rgb_input),
        reinterpret_cast<__half*>(gradients + NetworkLayout<Shape>::network_parameter_layout.rgb_param_offset),
        cublaslt_workspace);
    ...
    cuda_detail::cublaslt_matmul(
        ...
        cublaslt,
        "rgb input gradients",
        operation_descriptors[3],
        a_descriptors[3],
        b_descriptors[3],
        output_descriptors[3],
        algorithms[3],
        reinterpret_cast<const __half*>(params + NetworkLayout<Shape>::network_parameter_layout.rgb_param_offset),
        reinterpret_cast<const __half*>(rgb_backward_hidden) + (NetworkLayout<Shape>::rgb_hidden_layers - 1u) * hidden_layer_stride,
        reinterpret_cast<__half*>(rgb_input_gradients),
        cublaslt_workspace);

    ::cuda::launch(stream, linear_config, cuda_detail::add_density_gradient_kernel, NetworkLayout<Shape>::network_batch_size, reinterpret_cast<const __half*>(network_output_gradients), reinterpret_cast<__half*>(rgb_input_gradients));

    ::cuda::launch(stream, backward_config, cuda_detail::mlp_backward_hidden_64_relu_kernel<nvcuda::wmma::col_major, NetworkLayout<Shape>::density_hidden_layers>, NetworkLayout<Shape>::network_batch_size, reinterpret_cast<const __half*>(rgb_input_gradients), reinterpret_cast<const __half*>(params + NetworkLayout<Shape>::network_parameter_layout.density_param_offset), reinterpret_cast<const __half*>(density_forward_hidden), reinterpret_cast<__half*>(density_backward_hidden), NetworkLayout<Shape>::network_batch_size);

    cuda_detail::cublaslt_matmul(
        stream,
        cublaslt,
        "density last weight gradients",
        operation_descriptors[4],
        a_descriptors[4],
        b_descriptors[4],
        output_descriptors[4],
        algorithms[4],
        reinterpret_cast<const __half*>(rgb_input_gradients),
        reinterpret_cast<const __half*>(density_forward_hidden),
        reinterpret_cast<__half*>(gradients + NetworkLayout<Shape>::network_parameter_layout.density_output_weight_offset),
        cublaslt_workspace);

    cuda_detail::cublaslt_matmul(
        stream,
        cublaslt,
        "density first weight gradients",
        operation_descriptors[5],
        a_descriptors[5],
        b_descriptors[5],
        output_descriptors[5],
        algorithms[5],
        reinterpret_cast<const __half*>(density_backward_hidden),
        reinterpret_cast<const __half*>(density_input),
        reinterpret_cast<__half*>(gradients + NetworkLayout<Shape>::network_parameter_layout.density_param_offset),
        cublaslt_workspace);

    cuda_detail::cublaslt_matmul(
        stream,
        cublaslt,
        "density input gradients",
        operation_descriptors[6],
        a_descriptors[6],
        b_descriptors[6],
        output_descriptors[6],
        algorithms[6],
        reinterpret_cast<const __half*>(params + NetworkLayout<Shape>::network_parameter_layout.density_param_offset),
        reinterpret_cast<const __half*>(density_backward_hidden),
        reinterpret_cast<__half*>(density_input_gradients),
        cublaslt_workspace);
    ...
    constexpr std::uint32_t grid_threads = (NetworkLayout<Shape>::network_batch_size * NetworkLayout<Shape>::grid_features_per_level / 2u + 256u - 1u) / 256u;
    constexpr dim3 grid_blocks{grid_threads, NetworkLayout<Shape>::grid_n_levels, 1u};
    ::cuda::launch(stream, ::cuda::make_config(::cuda::make_hierarchy(::cuda::grid_dims(grid_blocks), ::cuda::block_dims<256u>())), cuda_detail::encode_grid_backward_kernel, NetworkLayout<Shape>::network_batch_size, NetworkLayout<Shape>::network_parameter_layout.grid_offsets, sample_coords, reinterpret_cast<const __half*>(density_input_gradients), reinterpret_cast<__half*>(gradients + NetworkLayout<Shape>::network_parameter_layout.grid_param_offset));
}
...
void NetworkKernels<Shape>::step_optimizer(const ::cuda::stream_ref stream, float* const params_full_precision, std::uint16_t* const params, const std::uint16_t* const gradients, float* const first_moments, float* const second_moments, std::uint32_t* const param_steps) {
    ...
    constexpr std::uint32_t blocks = (NetworkLayout<Shape>::network_parameter_layout.total_param_count + 128u - 1u) / 128u;
    ::cuda::launch(stream, ::cuda::make_config(::cuda::make_hierarchy(::cuda::grid_dims(blocks), ::cuda::block_dims<128u>())), cuda_detail::adam_step_kernel, params_full_precision, reinterpret_cast<__half*>(params), reinterpret_cast<const __half*>(gradients), first_moments, second_moments, param_steps);
}
