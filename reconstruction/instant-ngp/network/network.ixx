...
private:
    static consteval std::size_t make_parameter_count() {
        std::uint64_t grid_positions = 0u;
        std::uint64_t resolution = Shape.grid.base_resolution;
        ...
        for (std::uint32_t level = 0u; level < Shape.grid.level_count; ++level) {
            const std::uint64_t dense = resolution * resolution * resolution;
            const std::uint64_t bounded = dense < maximum_positions ? dense : maximum_positions;
            grid_positions += (std::min)(((bounded + 7u) / 8u) * 8u, hashmap_size);
            resolution *= Shape.grid.resolution_scale;
        }

        const std::uint64_t grid_parameters = grid_positions * Shape.grid.features_per_level;
        const std::uint64_t grid_output_width = static_cast<std::uint64_t>(Shape.grid.level_count) * Shape.grid.features_per_level;
        const std::uint64_t direction_output_width = static_cast<std::uint64_t>(Shape.direction.degree) * Shape.direction.degree;
        const std::uint64_t density_parameters = Shape.density.width * grid_output_width + (Shape.density.hidden_layer_count - 1u) * Shape.density.width * Shape.density.width + Shape.density.output_width * Shape.density.width;
        const std::uint64_t color_input_width = Shape.density.output_width + direction_output_width;
        const std::uint64_t color_parameters = Shape.color.width * color_input_width + (Shape.color.hidden_layer_count - 1u) * Shape.color.width * Shape.color.width + Shape.color.output_width * Shape.color.width;
        return static_cast<std::size_t>(grid_parameters + density_parameters + color_parameters);
    }
    ...
    inline static constexpr std::size_t parameter_count = make_parameter_count();
