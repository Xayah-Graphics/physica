...
namespace physica::deformables::cloth::cuda_detail {
    namespace {
        ...
        __device__ Vector<float> spring_force(const Vector<float> first_position, const Vector<float> second_position, const Vector<float> first_velocity, const Vector<float> second_velocity, const float stiffness, const float damping, const float rest_length) {
            const Vector<float> displacement      = second_position - first_position;
            const float length                    = sqrtf(dot(displacement, displacement));
            const Vector<float> direction         = displacement / length;
            const Vector<float> relative_velocity = second_velocity - first_velocity;
            const float magnitude                 = stiffness * (length - rest_length) + damping * dot(relative_velocity, direction);
            return magnitude * direction;
        }

        __device__ Vector<float> spring_force_tangent(const Vector<float> first_position, const Vector<float> second_position, const Vector<float> first_velocity, const Vector<float> second_velocity, const Vector<float> first_position_tangent, const Vector<float> second_position_tangent, const Vector<float> first_velocity_tangent, const Vector<float> second_velocity_tangent, const float stiffness, const float damping, const float rest_length, const float stiffness_tangent, const float damping_tangent, const float rest_length_tangent) {
            const Vector<float> displacement              = second_position - first_position;
            const Vector<float> displacement_tangent      = second_position_tangent - first_position_tangent;
            const float length                            = sqrtf(dot(displacement, displacement));
            const Vector<float> direction                 = displacement / length;
            const float length_tangent                    = dot(direction, displacement_tangent);
            const Vector<float> direction_tangent         = (displacement_tangent - length_tangent * direction) / length;
            const Vector<float> relative_velocity         = second_velocity - first_velocity;
            const Vector<float> relative_velocity_tangent = second_velocity_tangent - first_velocity_tangent;
        ...
        __device__ void spring_vjp(const Vector<float> first_position, const Vector<float> second_position, const Vector<float> first_velocity, const Vector<float> second_velocity, const float stiffness, const float damping, const float rest_length, const Vector<double> force_adjoint, Vector<double>& displacement_adjoint, Vector<double>& relative_velocity_adjoint, double& stiffness_adjoint, double& damping_adjoint, double& rest_length_adjoint) {
            const Vector<double> displacement{static_cast<double>(second_position.x) - first_position.x, static_cast<double>(second_position.y) - first_position.y, static_cast<double>(second_position.z) - first_position.z};
            const double length            = sqrt(dot(displacement, displacement));
            const Vector<double> direction = displacement / length;
            const Vector<double> relative_velocity{static_cast<double>(second_velocity.x) - first_velocity.x, static_cast<double>(second_velocity.y) - first_velocity.y, static_cast<double>(second_velocity.z) - first_velocity.z};
            ...
            Vector<double> direction_adjoint = magnitude * force_adjoint;
        ...
        __device__ Vector<float> gathered_force(const std::uint32_t particle, const ConstFieldView<float> positions, const ConstFieldView<float> velocities, const SpringTopologyView topology, const SpringParametersView parameters) {
            Vector<float> result{};
            for (std::uint32_t entry = topology.offsets[particle]; entry < topology.offsets[particle + 1u]; ++entry) {
                ...
                const Vector<float> force  = spring_force(load(positions, topology.first[spring]), load(positions, topology.second[spring]), load(velocities, topology.first[spring]), load(velocities, topology.second[spring]), parameters.stiffnesses[spring], parameters.dampings[spring], parameters.rest_lengths[spring]);
                result                     = result + (topology.first[spring] == particle ? force : -1.0F * force);
            }
            return result;
        }

        __device__ Vector<float> gathered_force_tangent(const std::uint32_t particle, const ConstFieldView<float> positions, const ConstFieldView<float> velocities, const ConstFieldView<float> position_tangent, const ConstFieldView<float> velocity_tangent, const SpringTopologyView topology, const SpringParametersView parameters, const SpringParametersView parameter_tangent) {
            Vector<float> result{};
            for (std::uint32_t entry = topology.offsets[particle]; entry < topology.offsets[particle + 1u]; ++entry) {
                const std::uint32_t spring  = topology.indices[entry];
                const std::uint32_t first   = topology.first[spring];
                const std::uint32_t second  = topology.second[spring];
                const Vector<float> tangent = spring_force_tangent(load(positions, first), load(positions, second), load(velocities, first), load(velocities, second), load(position_tangent, first), load(position_tangent, second), load(velocity_tangent, first), load(velocity_tangent, second), parameters.stiffnesses[spring], parameters.dampings[spring], parameters.rest_lengths[spring], parameter_tangent.stiffnesses[spring], parameter_tangent.dampings[spring], parameter_tangent.rest_lengths[spring]);
                result                      = result + (first == particle ? tangent : -1.0F * tangent);
            }
            return result;
        }

        __device__ void gathered_state_vjp(const std::uint32_t particle, const ConstFieldView<float> positions, const ConstFieldView<float> velocities, const ConstFieldView<double> force_adjoint, const SpringTopologyView topology, const SpringParametersView parameters, Vector<double>& position_adjoint, Vector<double>& velocity_adjoint) {
            for (std::uint32_t entry = topology.offsets[particle]; entry < topology.offsets[particle + 1u]; ++entry) {
                const std::uint32_t spring = topology.indices[entry];
                const std::uint32_t first  = topology.first[spring];
                const std::uint32_t second = topology.second[spring];
                Vector<double> displacement_adjoint;
                Vector<double> relative_velocity_adjoint;
        ...
        __global__ void force_forward_kernel(const std::uint32_t particle_count, const Vector<float> gravity, const ConstFieldView<float> positions, const ConstFieldView<float> velocities, const ConstFieldView<float> controls, const float* masses, const SpringTopologyView stretch_topology, const SpringParametersView stretch_parameters, const SpringTopologyView bending_topology, const SpringParametersView bending_parameters, const FieldView<float> forces) {
        ...
        __global__ void force_jvp_kernel(const std::uint32_t particle_count, const Vector<float> gravity, const ConstFieldView<float> positions, const ConstFieldView<float> velocities, const ConstFieldView<float> control_tangent, const ConstFieldView<float> position_tangent, const ConstFieldView<float> velocity_tangent, const float* mass_tangent, const SpringTopologyView stretch_topology, const SpringParametersView stretch_parameters, const SpringParametersView stretch_tangent, const SpringTopologyView bending_topology, const SpringParametersView bending_parameters, const SpringParametersView bending_tangent, const FieldView<float> output) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
        ...
        __global__ void force_state_vjp_kernel(const std::uint32_t particle_count, const Vector<float> gravity, const ConstFieldView<float> positions, const ConstFieldView<float> velocities, const ConstFieldView<double> force_adjoint, const SpringTopologyView stretch_topology, const SpringParametersView stretch_parameters, const SpringTopologyView bending_topology, const SpringParametersView bending_parameters, const FieldView<double> position_adjoint, const FieldView<double> velocity_adjoint, const FieldView<double> control_adjoint, double* mass_adjoint) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
            Vector<double> local_position_adjoint{};
            Vector<double> local_velocity_adjoint{};
            ...
            const Vector<double> local_force_adjoint = load(force_adjoint, particle);
        ...
        __global__ void force_parameter_vjp_kernel(const std::uint32_t spring_count, const ConstFieldView<float> positions, const ConstFieldView<float> velocities, const ConstFieldView<double> force_adjoint, const SpringTopologyView topology, const SpringParametersView parameters, const SpringParameterAdjointView parameter_adjoint) {
    ...
    void force_jvp(const ::cuda::stream_ref stream, const std::uint32_t particle_count, const Vector<float> gravity, const ConstFieldView<float> positions, const ConstFieldView<float> velocities, const ConstFieldView<float> control_tangent, const ConstFieldView<float> position_tangent, const ConstFieldView<float> velocity_tangent, const float* mass_tangent, const SpringTopologyView stretch_topology, const SpringParametersView stretch_parameters, const SpringParametersView stretch_tangent, const SpringTopologyView bending_topology, const SpringParametersView bending_parameters, const SpringParametersView bending_tangent, const FieldView<float> force_tangent) {
    ...
    void force_parameter_vjp(const ::cuda::stream_ref stream, const std::uint32_t spring_count, const ConstFieldView<float> positions, const ConstFieldView<float> velocities, const ConstFieldView<double> force_adjoint, const SpringTopologyView topology, const SpringParametersView parameters, const SpringParameterAdjointView parameter_adjoint) {
