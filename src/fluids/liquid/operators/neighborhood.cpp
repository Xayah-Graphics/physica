...
namespace physica::fluids::liquid::operators {
    Neighborhood UniformGridNeighborhood::allocate_cache(const Domain& domain) const {
    ...
    UniformGridNeighborhood::Workspace UniformGridNeighborhood::allocate_workspace(const Domain& domain) const {
        return {
            .unsorted_keys             = ::cuda::device_buffer<std::uint64_t>{domain.stream, ::cuda::device_default_memory_pool(domain.stream.device()), domain.configuration.particle_count, ::cuda::no_init},
            .unsorted_particle_indices = ::cuda::device_buffer<std::uint32_t>{domain.stream, ::cuda::device_default_memory_pool(domain.stream.device()), domain.configuration.particle_count, ::cuda::no_init},
            .unsorted_boundary_keys    = ::cuda::device_buffer<std::uint64_t>{domain.stream, ::cuda::device_default_memory_pool(domain.stream.device()), domain.configuration.boundary_particles.size(), ::cuda::no_init},
            .unsorted_boundary_indices = ::cuda::device_buffer<std::uint32_t>{domain.stream, ::cuda::device_default_memory_pool(domain.stream.device()), domain.configuration.boundary_particles.size(), ::cuda::no_init},
            .sort_scratch              = ::cuda::device_buffer<std::byte>{domain.stream, ::cuda::device_default_memory_pool(domain.stream.device()), cuda_detail::radix_sort_storage_size(domain.configuration.particle_count), ::cuda::no_init},
            .boundary_sort_scratch     = ::cuda::device_buffer<std::byte>{domain.stream, ::cuda::device_default_memory_pool(domain.stream.device()), cuda_detail::radix_sort_storage_size(static_cast<std::uint32_t>(domain.configuration.boundary_particles.size())), ::cuda::no_init},
        };
    }

    void UniformGridNeighborhood::build(const Domain& domain, const std::uint64_t step_index, const VectorField<float>& positions, Neighborhood& neighborhood, Workspace& workspace) const {
        const DomainConfiguration& configuration = domain.configuration;
