...
namespace physica::fluids::liquid::cuda_detail {
    namespace {
        ...
        template <bool Poly6>
        __device__ float density_kernel(const Float3 displacement, const float support_radius) {
            if constexpr (Poly6) return poly6(displacement, support_radius);
            else return cubic(displacement, support_radius);
        }

        template <bool Poly6>
        __device__ Float3 density_kernel_gradient(const Float3 displacement, const float support_radius) {
            if constexpr (Poly6) return poly6_gradient(displacement, support_radius);
            else return cubic_gradient(displacement, support_radius);
        }
                ...
                for (int offset_y = -1; offset_y <= 1; ++offset_y)
                    for (int offset_z = -1; offset_z <= 1; ++offset_z) {
                        ...
                        for (std::uint32_t sorted = range.first; sorted < range.last; ++sorted) {
                            ...
                            result += parameters.masses[neighbor] * density_kernel<Poly6>(subtract(position, load(positions, neighbor)), support_radius);
                        }
                        for (std::uint32_t sorted = range.boundary_first; sorted < range.boundary_last; ++sorted) {
                            ...
                            result += parameters.rest_densities[particle] * boundary.volumes[neighbor] * density_kernel<Poly6>(subtract(position, boundary_position(boundary, neighbor)), support_radius);
                        }
                    }
                ...
                for (int offset_y = -1; offset_y <= 1; ++offset_y)
                    for (int offset_z = -1; offset_z <= 1; ++offset_z) {
                        ...
                        for (std::uint32_t sorted = range.first; sorted < range.last; ++sorted) {
                            ...
                            result += parameter_tangent.masses[neighbor] * density_kernel<Poly6>(displacement, support_radius) + parameters.masses[neighbor] * dot(density_kernel_gradient<Poly6>(displacement, support_radius), subtract(tangent, load(position_tangent, neighbor)));
                        }
                        for (std::uint32_t sorted = range.boundary_first; sorted < range.boundary_last; ++sorted) {
                            const std::uint32_t neighbor = neighborhood.sorted_boundary_indices[sorted];
                ...
                for (int offset_y = -1; offset_y <= 1; ++offset_y)
                    for (int offset_z = -1; offset_z <= 1; ++offset_z) {
                        ...
                        for (std::uint32_t sorted = range.first; sorted < range.last; ++sorted) {
                            ...
                            const Float3 gradient     = density_kernel_gradient<Poly6>(displacement, support_radius);
                        ...
                        for (std::uint32_t sorted = range.boundary_first; sorted < range.boundary_last; ++sorted) {
                            ...
                            position_contribution        = add(position_contribution, scale(density_kernel_gradient<Poly6>(displacement, support_radius), output_adjoint * parameters.rest_densities[particle] * volume));
