...
namespace physica::fluids::liquid::cuda_detail {
    namespace {
        ...
        __global__ void key_kernel(const std::uint32_t count, const float support_radius, const Box domain, const float time, const bool moving_positions, const ConstVectorView<float> positions, const ConstVectorView<float> velocities, std::uint64_t* keys, std::uint32_t* indices) {
    ...
    void build_neighborhood(const ::cuda::stream_ref stream, const std::uint32_t particle_count, const std::uint32_t boundary_count, const float support_radius, const float time_step, const std::uint64_t step_index, const Box domain, const ConstVectorView<float> positions, const ConstVectorView<float> boundary_positions, const ConstVectorView<float> boundary_velocities, std::uint64_t* unsorted_keys, std::uint32_t* unsorted_particle_indices, std::uint64_t* unsorted_boundary_keys, std::uint32_t* unsorted_boundary_indices, void* sort_scratch, std::size_t sort_scratch_bytes, void* boundary_sort_scratch, std::size_t boundary_sort_scratch_bytes, std::uint64_t* sorted_keys, std::uint32_t* sorted_particle_indices, std::uint32_t* cell_offsets, std::uint64_t* sorted_boundary_keys, std::uint32_t* sorted_boundary_indices, std::uint32_t* boundary_cell_offsets) {
        ...
        ::cuda::launch(stream, ::cuda::distribute<block_size>(boundary_count), key_kernel, boundary_count, support_radius, domain, time, true, boundary_positions, boundary_velocities, unsorted_boundary_keys, unsorted_boundary_indices);
