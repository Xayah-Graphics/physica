...
namespace physica::optimization {
    struct LbfgsbBackendAccess final {
        [[nodiscard]] static cuda_backend::Storage storage(Lbfgsb& optimizer) {
            return {
    ...
    Lbfgsb::Lbfgsb(const ::cuda::stream_ref next_stream, LbfgsbConfiguration next_configuration, const std::span<const double> initial_parameters, const std::span<const double> next_lower_bounds, const std::span<const double> next_upper_bounds)
        : configuration(std::move(next_configuration)), parameter_count(initial_parameters.size()), parameters(next_stream, ::cuda::device_default_memory_pool(next_stream.device()), parameter_count, ::cuda::no_init), stream(next_stream), lower_bounds(next_stream, ::cuda::device_default_memory_pool(next_stream.device()), parameter_count, ::cuda::no_init), upper_bounds(next_stream, ::cuda::device_default_memory_pool(next_stream.device()), parameter_count, ::cuda::no_init), gradient(next_stream, ::cuda::device_default_memory_pool(next_stream.device()), parameter_count, ::cuda::no_init), trial_parameters(next_stream, ::cuda::device_default_memory_pool(next_stream.device()), parameter_count, ::cuda::no_init), correction_steps(next_stream, ::cuda::device_default_memory_pool(next_stream.device()), static_cast<std::size_t>(configuration.memory) * parameter_count, ::cuda::no_init),
          ...
          hessian_conjugate(next_stream, ::cuda::device_default_memory_pool(next_stream.device()), parameter_count, ::cuda::no_init), breakpoints(next_stream, ::cuda::device_default_memory_pool(next_stream.device()), parameter_count, ::cuda::no_init), sorted_breakpoints(next_stream, ::cuda::device_default_memory_pool(next_stream.device()), parameter_count, ::cuda::no_init), breakpoint_indices(next_stream, ::cuda::device_default_memory_pool(next_stream.device()), parameter_count, ::cuda::no_init), sorted_breakpoint_indices(next_stream, ::cuda::device_default_memory_pool(next_stream.device()), parameter_count, ::cuda::no_init), free_mask(next_stream, ::cuda::device_default_memory_pool(next_stream.device()), parameter_count, ::cuda::no_init), sort_scratch(next_stream, ::cuda::device_default_memory_pool(next_stream.device()), cuda_backend::sort_scratch_size(static_cast<std::uint32_t>(parameter_count)), ::cuda::no_init),
          status(next_stream, ::cuda::device_default_memory_pool(next_stream.device()), std::max({sizeof(cuda_backend::SearchResult), sizeof(cuda_backend::GradientMetrics), sizeof(cuda_backend::AcceptanceResult)}), ::cuda::no_init) {
        ...
        cuda_backend::initialize(stream, static_cast<std::uint32_t>(parameter_count), LbfgsbBackendAccess::storage(*this));
        stream.sync();
    }
    ...
    LbfgsbGradientMetrics Lbfgsb::submit(const double* next_objective_device, const ::cuda::std::span<const double> next_gradient) {
        ...
        const cuda_backend::GradientMetrics device_result = cuda_backend::gradient_metrics(stream, static_cast<std::uint32_t>(parameter_count), request_parameters, next_gradient.data(), LbfgsbBackendAccess::storage(*this));
    ...
    void Lbfgsb::begin_line_search(const double projected_gradient_norm) {
        ...
        const cuda_backend::SearchResult search = cuda_backend::prepare_direction(stream, static_cast<std::uint32_t>(parameter_count), configuration.memory, correction_count, correction_head, LbfgsbBackendAccess::storage(*this));
    ...
    void Lbfgsb::set_trial() {
        cuda_backend::set_trial(stream, static_cast<std::uint32_t>(parameter_count), step_length, LbfgsbBackendAccess::storage(*this));
    }
    ...
    void Lbfgsb::accept_trial(const double next_objective, const ::cuda::std::span<const double> next_gradient, const LbfgsbGradientMetrics metrics) {
        ...
        const cuda_backend::AcceptanceResult acceptance = cuda_backend::accept_trial(stream, static_cast<std::uint32_t>(parameter_count), correction_slot, next_gradient.data(), LbfgsbBackendAccess::storage(*this));
