...
namespace physica::optimization::cuda_backend {
    namespace {
        constexpr std::uint32_t block_size = 256u;

        __device__ double clamp(const double value, const double lower, const double upper) {
        ...
        __device__ double block_min(const double value, double* reduction) {
            ...
            __syncthreads();
            for (std::uint32_t offset = blockDim.x / 2u; offset != 0u; offset /= 2u) {
                if (threadIdx.x < offset) reduction[threadIdx.x] = fmin(reduction[threadIdx.x], reduction[threadIdx.x + offset]);
                __syncthreads();
            }
            const double result = reduction[0];
            __syncthreads();
        ...
        __global__ void initialize_kernel(const std::uint32_t parameter_count, const Storage storage) {
            ...
            if (parameter < parameter_count) storage.parameters[parameter] = clamp(storage.parameters[parameter], storage.lower_bounds[parameter], storage.upper_bounds[parameter]);
        }
        ...
        __global__ void gradient_metrics_kernel(const std::uint32_t parameter_count, const double* parameters, const double* gradient, const Storage storage) {
            ...
            for (std::uint32_t parameter = threadIdx.x; parameter < parameter_count; parameter += blockDim.x) {
                ...
                const double projected = parameters[parameter] - clamp(parameters[parameter] - gradient[parameter], storage.lower_bounds[parameter], storage.upper_bounds[parameter]);
                projected_squared += projected * projected;
            }
            const double gradient_norm  = sqrt(block_sum(gradient_squared, gradient_reduction));
            const double projected_norm = sqrt(block_sum(projected_squared, projected_reduction));
        ...
        __global__ void search_direction_kernel(const std::uint32_t parameter_count, const std::uint32_t memory, const std::uint32_t correction_count, const std::uint32_t correction_head, const Storage storage) {
            ...
            for (std::uint32_t parameter = threadIdx.x; parameter < parameter_count; parameter += blockDim.x) {
                storage.cauchy[parameter]       = clamp(storage.parameters[parameter] + cauchy_time * storage.path[parameter], storage.lower_bounds[parameter], storage.upper_bounds[parameter]);
            ...
            for (std::uint32_t parameter = threadIdx.x; parameter < parameter_count; parameter += blockDim.x) {
                if (storage.subspace_direction[parameter] > 0.0) local_step = fmin(local_step, (storage.upper_bounds[parameter] - storage.cauchy[parameter]) / storage.subspace_direction[parameter]);
                if (storage.subspace_direction[parameter] < 0.0) local_step = fmin(local_step, (storage.lower_bounds[parameter] - storage.cauchy[parameter]) / storage.subspace_direction[parameter]);
            }
            const double subspace_step = block_min(local_step, reduction);
            for (std::uint32_t parameter = threadIdx.x; parameter < parameter_count; parameter += blockDim.x) {
                const double candidate       = clamp(storage.cauchy[parameter] + subspace_step * storage.subspace_direction[parameter], storage.lower_bounds[parameter], storage.upper_bounds[parameter]);
        ...
        __global__ void set_trial_kernel(const std::uint32_t parameter_count, const double step_length, const Storage storage) {
            const std::uint32_t parameter = blockIdx.x * blockDim.x + threadIdx.x;
            if (parameter < parameter_count) storage.trial_parameters[parameter] = clamp(storage.parameters[parameter] + step_length * storage.direction[parameter], storage.lower_bounds[parameter], storage.upper_bounds[parameter]);
        }
