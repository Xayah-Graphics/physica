...
namespace physica::reconstruction::instant_ngp::cuda_detail {
    ...
    inline __device__ float3 srgb_to_linear(const float3 value) {
        return {
    ...
    inline __device__ float3 linear_to_srgb(const float3 value) {
        constexpr float inverse_gamma = 1.0f / 2.4f;
        return {
            value.x < 0.0031308f ? 12.92f * value.x : 1.055f * powf(value.x, inverse_gamma) - 0.055f,
            value.y < 0.0031308f ? 12.92f * value.y : 1.055f * powf(value.y, inverse_gamma) - 0.055f,
            value.z < 0.0031308f ? 12.92f * value.z : 1.055f * powf(value.z, inverse_gamma) - 0.055f,
        };
    }
...
namespace physica::reconstruction::instant_ngp::kernels {
    ...
    void RenderingKernels<Shape>::pad_compacted_training_batch(const ::cuda::stream_ref stream, const std::uint32_t* const compacted_sample_counter, float* const compacted_sample_coords, std::uint16_t* const network_output_gradients) {
        ...
        ::cuda::launch(stream, ::cuda::distribute<128u>(gradient_elements), cuda_detail::pad_rollover_network_output_gradients_kernel, compacted_sample_counter, reinterpret_cast<__half*>(network_output_gradients));

        constexpr std::uint32_t coord_elements = RenderingLayout<Shape>::network_batch_size * 7u;
        ::cuda::launch(stream, ::cuda::distribute<128u>(coord_elements), cuda_detail::pad_rollover_coords_kernel, compacted_sample_counter, compacted_sample_coords);
    }
    ...
    void RenderingKernels<Shape>::accumulate_evaluation_loss(const ::cuda::stream_ref stream, const std::uint32_t tile_pixels, const std::uint32_t pixel_offset, const std::uint32_t image_index, const std::uint32_t width, const std::uint32_t height, const std::uint8_t* const pixels, const std::uint32_t* const numsteps, const float* const sample_coords, const std::uint16_t* const network_output, double* const loss_sum) {
        ::cuda::launch(stream, ::cuda::distribute<128u>(tile_pixels), cuda_detail::accumulate_evaluation_loss_kernel, tile_pixels, pixel_offset, image_index, width, height, pixels, numsteps, sample_coords, reinterpret_cast<const __half*>(network_output), loss_sum);
    }
