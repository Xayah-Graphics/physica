...
module physica.deformables.cloth.integrators.semi_implicit_euler;
...
namespace physica::deformables::cloth::integrators {
    ...
    SemiImplicitEuler::Cache SemiImplicitEuler::allocate_cache(const Model<float>&) const {
        return {};
    }

    SemiImplicitEuler::Workspace SemiImplicitEuler::allocate_workspace(const Model<float>&) const {
        return {};
    }

    SemiImplicitEuler::TangentWorkspace SemiImplicitEuler::allocate_tangent_workspace(const Model<float>&) const {
        return {};
    }

    SemiImplicitEuler::AdjointWorkspace SemiImplicitEuler::allocate_adjoint_workspace(const Model<float>&) const {
        return {};
    }

    void SemiImplicitEuler::forward(const Model<float>& model, const simulation::VectorField<float>& positions, const simulation::VectorField<float>& velocities, const simulation::ScalarField<float>& masses, const simulation::VectorField<float>& forces, simulation::VectorField<float>& integrated_positions, simulation::VectorField<float>& integrated_velocities, Cache&, Workspace&) const {
