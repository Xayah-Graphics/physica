...
module physica.deformables.cloth.solvers.corotated_fem;
...
namespace physica::deformables::cloth::solvers::corotated_fem {
    ...
    void Solver::forward(const Model<float>& model, const State<float>& state, const Control<float>& control, const Parameters& parameters, State<float>& next_state, StepCache& cache, Workspace& workspace) const {
        ...
        for (std::uint32_t iteration = 0u; iteration < newton_iteration_count; ++iteration) {
            ...
            kernels::evaluate_rank_step_limits(model.stream, triangle_count, rank_safety_fraction, model.topology.device.triangles.first.values.data(), model.topology.device.triangles.second.values.data(), model.topology.device.triangles.third.values.data(), simulation::view(material_u_gradients), simulation::view(material_v_gradients), simulation::view(next_state.positions), simulation::view(workspace.newton_direction), cache.triangle_rank_step_limits.values.data());
            kernels::reduce_minimum_float(model.stream, triangle_count, cache.triangle_rank_step_limits.values.data(), cache.maximum_rank_safe_step.values.data());
    ...
    void Solver::evaluate_system(const Model<float>& model, const Parameters&, const simulation::VectorField<float>& positions, StepCache& cache, Workspace& workspace) const {
        ...
        kernels::reduce_minimum_double(model.stream, particle_count, workspace.gershgorin_lower_bounds.values.data(), cache.minimum_gershgorin_bound.values.data());
