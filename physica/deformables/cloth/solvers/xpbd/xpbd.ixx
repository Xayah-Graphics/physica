...
export module physica.deformables.cloth.solvers.xpbd;
...
export namespace physica::deformables::cloth::solvers::xpbd {
    struct Solver final {
    ...
    private:
        [[nodiscard]] static std::vector<Edge> build_constraints(const Model<float>& model);
        ...
        simulation::ScalarField<std::uint32_t> colored_constraints;
        simulation::ScalarField<std::uint32_t> constraint_first;
        simulation::ScalarField<std::uint32_t> constraint_second;
        simulation::ScalarField<float> rest_lengths;
        simulation::ScalarField<float> compliances;
