...
namespace physica::deformables::cloth::solvers::discrete_shells {
    ...
    Solver::Solver(const Model<float>& model, const Configuration& configuration, HostData host_data)
        : time_step(configuration.time_step), newton_iteration_count(configuration.newton_iteration_count), line_search_candidate_count(configuration.line_search_candidate_count), gravity(configuration.gravity), length_stiffness(configuration.length_stiffness), area_stiffness(configuration.area_stiffness), bending_stiffness(configuration.bending_stiffness), bending_damping(configuration.bending_damping), hessian_positive_margin(configuration.hessian_positive_margin), armijo_coefficient(configuration.armijo_coefficient), pattern(std::move(host_data.pattern)), block_solver(model, {.iteration_count = configuration.pcg_iteration_count}), edge_rest_lengths(model.stream, host_data.edge_rest_lengths.size()), triangle_rest_areas(model.stream, host_data.triangle_rest_areas.size()), hinge_rest_angles(model.stream, host_data.hinge_rest_angles.size()), hinge_weights(model.stream, host_data.hinge_weights.size()), energy_hessian_contribution_offsets(model.stream, pattern.energy_hessian_contribution_offsets.size()),
          energy_hessian_contributions(model.stream, pattern.energy_hessian_contributions.size()), damping_jacobian_contribution_offsets(model.stream, pattern.damping_jacobian_contribution_offsets.size()), damping_jacobian_contributions(model.stream, pattern.damping_jacobian_contributions.size()), fixed_vertex_mask(model.stream, host_data.fixed_vertex_mask.size()), fixed_positions(model.stream, host_data.fixed_positions.size()), line_search_steps(model.stream, host_data.line_search_steps.size()) {
