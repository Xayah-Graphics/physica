...
namespace physica::neural {
    ...
    MatmulRuntime::Plan::Plan(const cublasLtHandle_t handle, const PlanKey& source_key, const void* const bias, const void* const auxiliary, const std::size_t workspace_byte_count) : key{source_key} {
        ...
        if (const cublasStatus_t status = cublasLtMatmulDescCreate(&operation, CUBLAS_COMPUTE_32F_FAST_TF32, CUDA_R_32F); status != CUBLAS_STATUS_SUCCESS) throw std::runtime_error{std::string{"cuBLASLt operation descriptor: "} + cublasGetStatusString(status)};
        if (const cublasStatus_t status = cublasLtMatmulDescSetAttribute(operation, CUBLASLT_MATMUL_DESC_TRANSA, &transpose_a, sizeof(transpose_a)); status != CUBLAS_STATUS_SUCCESS) throw std::runtime_error{std::string{"cuBLASLt transpose A: "} + cublasGetStatusString(status)};
        if (const cublasStatus_t status = cublasLtMatmulDescSetAttribute(operation, CUBLASLT_MATMUL_DESC_TRANSB, &transpose_b, sizeof(transpose_b)); status != CUBLAS_STATUS_SUCCESS) throw std::runtime_error{std::string{"cuBLASLt transpose B: "} + cublasGetStatusString(status)};
        if (const cublasStatus_t status = cublasLtMatmulDescSetAttribute(operation, CUBLASLT_MATMUL_DESC_SM_COUNT_TARGET, &multiprocessor_count, sizeof(multiprocessor_count)); status != CUBLAS_STATUS_SUCCESS) throw std::runtime_error{std::string{"cuBLASLt SM count: "} + cublasGetStatusString(status)};
        if (const cublasStatus_t status = cublasLtMatmulDescSetAttribute(operation, CUBLASLT_MATMUL_DESC_EPILOGUE, &epilogue, sizeof(epilogue)); status != CUBLAS_STATUS_SUCCESS) throw std::runtime_error{std::string{"cuBLASLt epilogue: "} + cublasGetStatusString(status)};
        if (uses_bias(key.epilogue))
            if (const cublasStatus_t status = cublasLtMatmulDescSetAttribute(operation, CUBLASLT_MATMUL_DESC_BIAS_POINTER, &bias, sizeof(bias)); status != CUBLAS_STATUS_SUCCESS) throw std::runtime_error{std::string{"cuBLASLt bias: "} + cublasGetStatusString(status)};
        if (uses_auxiliary(key.epilogue)) {
            const std::int64_t auxiliary_leading = static_cast<std::int64_t>(key.columns);
            if (const cublasStatus_t status = cublasLtMatmulDescSetAttribute(operation, CUBLASLT_MATMUL_DESC_EPILOGUE_AUX_POINTER, &auxiliary, sizeof(auxiliary)); status != CUBLAS_STATUS_SUCCESS) throw std::runtime_error{std::string{"cuBLASLt auxiliary: "} + cublasGetStatusString(status)};
            if (const cublasStatus_t status = cublasLtMatmulDescSetAttribute(operation, CUBLASLT_MATMUL_DESC_EPILOGUE_AUX_LD, &auxiliary_leading, sizeof(auxiliary_leading)); status != CUBLAS_STATUS_SUCCESS) throw std::runtime_error{std::string{"cuBLASLt auxiliary leading dimension: "} + cublasGetStatusString(status)};
        }
        if (const cublasStatus_t status = cublasLtMatrixLayoutCreate(&a_layout, CUDA_R_32F, a_rows, a_columns, a_leading); status != CUBLAS_STATUS_SUCCESS) throw std::runtime_error{std::string{"cuBLASLt A layout: "} + cublasGetStatusString(status)};
        if (const cublasStatus_t status = cublasLtMatrixLayoutCreate(&b_layout, CUDA_R_32F, b_rows, b_columns, b_leading); status != CUBLAS_STATUS_SUCCESS) throw std::runtime_error{std::string{"cuBLASLt B layout: "} + cublasGetStatusString(status)};
        if (const cublasStatus_t status = cublasLtMatrixLayoutCreate(&output_layout, CUDA_R_32F, key.columns, key.rows, output_leading); status != CUBLAS_STATUS_SUCCESS) throw std::runtime_error{std::string{"cuBLASLt output layout: "} + cublasGetStatusString(status)};
        if (const cublasStatus_t status = cublasLtMatmulPreferenceCreate(&preference); status != CUBLAS_STATUS_SUCCESS) throw std::runtime_error{std::string{"cuBLASLt preference: "} + cublasGetStatusString(status)};
        if (const cublasStatus_t status = cublasLtMatmulPreferenceSetAttribute(preference, CUBLASLT_MATMUL_PREF_MAX_WORKSPACE_BYTES, &workspace_byte_count, sizeof(workspace_byte_count)); status != CUBLAS_STATUS_SUCCESS) throw std::runtime_error{std::string{"cuBLASLt workspace preference: "} + cublasGetStatusString(status)};
        if (const cublasStatus_t status = cublasLtMatmulPreferenceSetAttribute(preference, CUBLASLT_MATMUL_PREF_REDUCTION_SCHEME_MASK, &reduction_scheme, sizeof(reduction_scheme)); status != CUBLAS_STATUS_SUCCESS) throw std::runtime_error{std::string{"cuBLASLt reduction preference: "} + cublasGetStatusString(status)};
    ...
    void MatmulRuntime::execute(const MatmulRequest& request) {
        ...
        if (!plan->tuned) {
            ...
            for (const cublasLtMatmulAlgo_t& candidate : plan->candidates) {
                ...
                for (std::uint32_t repetition = 0u; repetition < 3u; ++repetition)
                    if (const cublasStatus_t status = cublasLtMatmul(handle, plan->operation, &alpha, request.b, plan->a_layout, request.a, plan->b_layout, &beta, tuning_output.data(), plan->output_layout, tuning_output.data(), plan->output_layout, &candidate, workspace.data(), workspace.size(), stream.get()); status != CUBLAS_STATUS_SUCCESS) throw std::runtime_error{std::string{"cuBLASLt tuning candidate: "} + cublasGetStatusString(status)};
                if (const cudaError_t status = cudaEventRecord(end, stream.get()); status != cudaSuccess) throw std::runtime_error{std::string{"CUDA tuning end record: "} + cudaGetErrorString(status)};
                if (const cudaError_t status = cudaEventSynchronize(end); status != cudaSuccess) throw std::runtime_error{std::string{"CUDA tuning synchronization: "} + cudaGetErrorString(status)};
                float milliseconds{};
        ...
        if (uses_bias(key.epilogue))
            if (const cublasStatus_t status = cublasLtMatmulDescSetAttribute(plan->operation, CUBLASLT_MATMUL_DESC_BIAS_POINTER, &request.bias, sizeof(request.bias)); status != CUBLAS_STATUS_SUCCESS) throw std::runtime_error{std::string{"cuBLASLt bias pointer: "} + cublasGetStatusString(status)};
        if (uses_auxiliary(key.epilogue))
