...
namespace physica::fluids::liquid::solvers::sph::kernels::dfsph {
    ...
    namespace {
        ...
        __global__ void projection_update_forward_kernel(const std::uint32_t particle_count, const float time_step, const float reference_gradient_norm, const device::ParticleParameterView particles, const float* densities, const float* target_densities, const float* previous_pressures, const float* predicted_densities, const float* projection_relaxation, float* pressures) {
            ...
            pressures[particle] = ::cuda::std::max(0.0F, previous_pressures[particle] + projection_relaxation[particle] * delta * (predicted_densities[particle] - target_densities[particle]));
        }
