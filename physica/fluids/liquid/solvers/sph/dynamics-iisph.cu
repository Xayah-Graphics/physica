...
namespace physica::fluids::liquid::solvers::sph::kernels::iisph {
    ...
    namespace {
        ...
        __global__ void jacobi_update_forward_kernel(const std::uint32_t particle_count, const float time_step, const float reference_gradient_norm, const device::ParticleParameterView particles, const float* densities, const float* previous_pressures, const float* predicted_densities, const float* jacobi_relaxation, float* pressures) {
            ...
            pressures[particle] = ::cuda::std::max(0.0F, previous_pressures[particle] + jacobi_relaxation[particle] * delta * (predicted_densities[particle] - particles.rest_densities[particle]));
        }
        ...
        __global__ void jacobi_update_vjp_kernel(const std::uint32_t particle_count, const float time_step, const float reference_gradient_norm, const device::ParticleParameterView particles, const float* densities, const float* previous_pressures, const float* predicted_densities, const float* jacobi_relaxation, const double* pressure_adjoint, const device::ParticleParameterAdjointView particle_adjoint, double* density_adjoint, double* previous_pressure_adjoint, double* predicted_density_adjoint, double* jacobi_relaxation_adjoint) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
