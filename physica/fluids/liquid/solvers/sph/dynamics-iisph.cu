...
namespace physica::fluids::liquid::sph::kernels::iisph {
    ...
    void launch_jacobi_update_forward(const ::cuda::stream_ref stream, const std::uint32_t particle_count, const float time_step, const float reference_gradient_norm, const device::ParticleParameterView particles, const float* densities, const float* previous_pressures, const float* predicted_densities, const float* jacobi_relaxation, float* pressures) {
        jacobi_update_forward_kernel<<<::cuda::ceil_div(particle_count, block_size), block_size, 0, stream.get()>>>(particle_count, time_step, reference_gradient_norm, particles, densities, previous_pressures, predicted_densities, jacobi_relaxation, pressures);
    }
