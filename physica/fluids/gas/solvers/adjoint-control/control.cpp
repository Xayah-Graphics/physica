...
namespace physica::fluids::gas::adjoint_control {
    ...
    void ControlSystem::forward(const Domain& domain, const std::uint32_t step, const ::cuda::std::span<const double> parameters, DenseControl& output) const {
        kernels::control_forward(domain.grid.fields.stream, device::discretization(domain.configuration), step, {configuration.lattice[0], configuration.lattice[1], configuration.lattice[2]}, configuration.gaussian_sigma, configuration.step_count, parameters.data(), field::view(output.force));
    }
    ...
    void ControlSystem::jvp(const Domain& domain, const std::uint32_t step, const ::cuda::std::span<const double> direction, DenseControlTangent& output_tangent) const {
        kernels::control_jvp(domain.grid.fields.stream, device::discretization(domain.configuration), step, {configuration.lattice[0], configuration.lattice[1], configuration.lattice[2]}, configuration.gaussian_sigma, configuration.step_count, direction.data(), field::view(output_tangent.force));
    }
    ...
    void ControlSystem::vjp(const Domain& domain, const std::uint32_t step, const DenseControlAdjoint& output_adjoint, const ::cuda::std::span<double> gradient) const {
        kernels::control_vjp(domain.grid.fields.stream, device::discretization(domain.configuration), step, {configuration.lattice[0], configuration.lattice[1], configuration.lattice[2]}, configuration.gaussian_sigma, configuration.step_count, field::view(output_adjoint.force), gradient.data());
    }
