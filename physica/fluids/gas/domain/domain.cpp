...
namespace physica::fluids::gas {
    ...
    Domain::Domain(DomainConfiguration next_configuration, const ::cuda::stream_ref source_stream)
        ...
        ::cuda::copy_bytes(grid.fields.stream, collider_indices, collider_ids.values);
        ...
        ::cuda::copy_bytes(grid.fields.stream, x_velocity, collider_velocity.x);
        ::cuda::copy_bytes(grid.fields.stream, y_velocity, collider_velocity.y);
        ::cuda::copy_bytes(grid.fields.stream, z_velocity, collider_velocity.z);
        grid.fields.stream.sync();
    }

    ScalarField<float> Domain::allocate_collider_field(const std::span<const float> values) const {
        ...
        ScalarField<float> result = grid.allocate_cell_field<float>();
        ::cuda::copy_bytes(grid.fields.stream, host_values, result.values);
        return result;
    }
