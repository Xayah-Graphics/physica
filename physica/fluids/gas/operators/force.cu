...
namespace physica::fluids::gas::operators::kernels {
    namespace {
        ...
        __global__ void combine_forward_kernel(const device::Discretization grid, const std::uint32_t* collider_ids, const field::VectorView<const float> physical, const field::VectorView<const float> control, const field::VectorView<float> total) {
        ...
        __global__ void combine_vjp_kernel(const device::Discretization grid, const std::uint32_t* collider_ids, const field::VectorView<const double> total_adjoint, const field::VectorView<double> physical_adjoint, const field::VectorView<double> control_adjoint) {
            const std::uint64_t index = static_cast<std::uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
            if (index >= fluids::grid::device::cell_count(grid.grid)) return;
        ...
        __global__ void density_buoyancy_forward_kernel(const device::Discretization grid, const std::uint32_t* collider_ids, const float buoyancy, const float* density, const field::VectorView<float> force) {
            const std::uint64_t index = static_cast<std::uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
            if (index >= fluids::grid::device::cell_count(grid.grid)) return;
            force.x[index] = 0.0F;
        ...
        __global__ void density_buoyancy_vjp_kernel(const device::Discretization grid, const std::uint32_t* collider_ids, const float buoyancy, const field::VectorView<const double> force_adjoint, double* density_adjoint) {
            const std::uint64_t index = static_cast<std::uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
        ...
        __global__ void buoyancy_forward_kernel(const device::Discretization grid, const std::uint32_t* collider_ids, const float* density, const float* temperature, const field::VectorView<const float> external_acceleration, const float* ambient_temperature, const float* density_buoyancy, const float* temperature_buoyancy, const field::VectorView<float> force) {
            const std::uint64_t index = static_cast<std::uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
            if (index >= fluids::grid::device::cell_count(grid.grid)) return;
        ...
        __global__ void buoyancy_jvp_kernel(const device::Discretization grid, const std::uint32_t* collider_ids, const float* density, const float* temperature, const float* density_tangent, const float* temperature_tangent, const field::VectorView<const float> external_tangent, const float* ambient, const float* density_buoyancy, const float* temperature_buoyancy, const float* ambient_tangent, const float* density_buoyancy_tangent, const float* temperature_buoyancy_tangent, const field::VectorView<float> output) {
            const std::uint64_t index = static_cast<std::uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
            if (index >= fluids::grid::device::cell_count(grid.grid)) return;
            if (collider_ids[index] != 0u) {
                output.x[index] = 0.0F;
        ...
        __global__ void centered_velocity_kernel(const device::Discretization grid, const std::uint32_t* collider_ids, const field::VectorView<const float> velocity, const field::VectorView<float> centered) {
            const std::uint64_t index = static_cast<std::uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
            if (index >= fluids::grid::device::cell_count(grid.grid)) return;
            int x, y, z;
        ...
        __global__ void curl_magnitude_kernel(const device::Discretization grid, const std::uint32_t* collider_ids, const field::VectorView<const float> centered, const field::VectorView<float> vorticity, float* magnitude) {
            const std::uint64_t index = static_cast<std::uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
            if (index >= fluids::grid::device::cell_count(grid.grid)) return;
            int x, y, z;
            fluids::grid::device::decode(index, grid.grid.nx, grid.grid.ny, x, y, z);
        ...
        __global__ void vorticity_normal_kernel(const device::Discretization grid, const std::uint32_t* collider_ids, const float* magnitude, const field::VectorView<float> normal, float* normalizer) {
            const std::uint64_t index = static_cast<std::uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
            if (index >= fluids::grid::device::cell_count(grid.grid)) return;
            int x, y, z;
            fluids::grid::device::decode(index, grid.grid.nx, grid.grid.ny, x, y, z);
        ...
        __global__ void add_vorticity_force_kernel(const device::Discretization grid, const std::uint32_t* collider_ids, const field::VectorView<const float> vorticity, const field::VectorView<const float> normal, const Parameter confinement, const field::VectorView<float> force) {
            const std::uint64_t index = static_cast<std::uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
            if (index >= fluids::grid::device::cell_count(grid.grid) || collider_ids[index] != 0u) return;
        ...
        __global__ void magnitude_tangent_kernel(const device::Discretization grid, const std::uint32_t* collider_ids, const field::VectorView<const float> vorticity, const field::VectorView<const float> vorticity_tangent, const float* magnitude, float* magnitude_tangent) {
            const std::uint64_t index = static_cast<std::uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
            if (index >= fluids::grid::device::cell_count(grid.grid)) return;
        ...
        __global__ void normal_tangent_kernel(const device::Discretization grid, const std::uint32_t* collider_ids, const float* magnitude_tangent, const field::VectorView<const float> normal, const float* normalizer, const field::VectorView<float> normal_tangent) {
            const std::uint64_t index = static_cast<std::uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
            if (index >= fluids::grid::device::cell_count(grid.grid)) return;
            int x, y, z;
            fluids::grid::device::decode(index, grid.grid.nx, grid.grid.ny, x, y, z);
        ...
        __global__ void vorticity_force_tangent_kernel(const device::Discretization grid, const std::uint32_t* collider_ids, const field::VectorView<const float> vorticity, const field::VectorView<const float> vorticity_tangent, const field::VectorView<const float> normal, const field::VectorView<const float> normal_tangent, const Parameter confinement, const ParameterTangent confinement_tangent, const field::VectorView<float> force_tangent) {
            const std::uint64_t index = static_cast<std::uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
            if (index >= fluids::grid::device::cell_count(grid.grid) || collider_ids[index] != 0u) return;
        ...
        __global__ void vorticity_force_reverse_kernel(const device::Discretization grid, const std::uint32_t* collider_ids, const field::VectorView<const float> vorticity, const field::VectorView<const float> normal, const Parameter confinement, const field::VectorView<const double> force_adjoint, const field::VectorView<double> vorticity_adjoint, const field::VectorView<double> normal_adjoint, const ParameterAdjoint confinement_adjoint) {
            const std::uint64_t index = static_cast<std::uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
            if (index >= fluids::grid::device::cell_count(grid.grid) || collider_ids[index] != 0u) return;
            const Vector3<float> cross{
                normal.y[index] * vorticity.z[index] - normal.z[index] * vorticity.y[index],
        ...
        __global__ void normal_reverse_kernel(const device::Discretization grid, const std::uint32_t* collider_ids, const field::VectorView<const float> normal, const float* normalizer, const field::VectorView<const double> normal_adjoint, double* magnitude_adjoint) {
            const std::uint64_t index = static_cast<std::uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
            if (index >= fluids::grid::device::cell_count(grid.grid) || collider_ids[index] != 0u) return;
            int x, y, z;
            fluids::grid::device::decode(index, grid.grid.nx, grid.grid.ny, x, y, z);
        ...
        __global__ void magnitude_reverse_kernel(const device::Discretization grid, const std::uint32_t* collider_ids, const field::VectorView<const float> vorticity, const float* magnitude, const double* magnitude_adjoint, const field::VectorView<double> vorticity_adjoint) {
            const std::uint64_t index = static_cast<std::uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
            if (index >= fluids::grid::device::cell_count(grid.grid) || collider_ids[index] != 0u) return;
        ...
        __global__ void curl_reverse_kernel(const device::Discretization grid, const std::uint32_t* collider_ids, const field::VectorView<const double> vorticity_adjoint, const field::VectorView<double> centered_adjoint) {
            const std::uint64_t index = static_cast<std::uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
            if (index >= fluids::grid::device::cell_count(grid.grid) || collider_ids[index] != 0u) return;
            int x, y, z;
            fluids::grid::device::decode(index, grid.grid.nx, grid.grid.ny, x, y, z);
        ...
        __global__ void centered_velocity_reverse_kernel(const device::Discretization grid, const std::uint32_t* collider_ids, const field::VectorView<const double> centered_adjoint, const field::VectorView<double> velocity_adjoint) {
            const std::uint64_t index = static_cast<std::uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
            if (index >= fluids::grid::device::cell_count(grid.grid) || collider_ids[index] != 0u) return;
            int x, y, z;
            fluids::grid::device::decode(index, grid.grid.nx, grid.grid.ny, x, y, z);
        ...
        void vorticity_forward_fields(const ::cuda::stream_ref stream, const device::Discretization grid, const std::uint32_t* collider_ids, const field::VectorView<const float> velocity, const VorticityView cache) {
            ...
            ::cuda::launch(stream, ::cuda::distribute<fluids::grid::device::block_size>(fluids::grid::device::cell_count(grid.grid)), curl_magnitude_kernel, grid, collider_ids, field::VectorView<const float>{cache.centered_velocity.x, cache.centered_velocity.y, cache.centered_velocity.z}, cache.vorticity, cache.magnitude.values);
        ...
        void vorticity_tangent_fields(const ::cuda::stream_ref stream, const device::Discretization grid, const std::uint32_t* collider_ids, const field::VectorView<const float> velocity_tangent, const ConstVorticityView cache, const VorticityTangentScratch tangent) {
            ...
            ::cuda::launch(stream, ::cuda::distribute<fluids::grid::device::block_size>(fluids::grid::device::cell_count(grid.grid)), curl_magnitude_kernel, grid, collider_ids, field::VectorView<const float>{tangent.centered_velocity.x, tangent.centered_velocity.y, tangent.centered_velocity.z}, tangent.vorticity, tangent.magnitude.values);
            ::cuda::launch(stream, ::cuda::distribute<fluids::grid::device::block_size>(fluids::grid::device::cell_count(grid.grid)), magnitude_tangent_kernel, grid, collider_ids, cache.vorticity, field::VectorView<const float>{tangent.vorticity.x, tangent.vorticity.y, tangent.vorticity.z}, cache.magnitude.values, tangent.magnitude.values);
        ...
        void vorticity_reverse_fields(const ::cuda::stream_ref stream, const device::Discretization grid, const std::uint32_t* collider_ids, const ConstVorticityView cache, const VorticityAdjointScratch scratch, const field::VectorView<double> velocity_adjoint) {
            ::cuda::launch(stream, ::cuda::distribute<fluids::grid::device::block_size>(fluids::grid::device::cell_count(grid.grid)), normal_reverse_kernel, grid, collider_ids, cache.normal, cache.normalizer.values, field::VectorView<const double>{scratch.normal.x, scratch.normal.y, scratch.normal.z}, scratch.magnitude.values);
            ...
            ::cuda::launch(stream, ::cuda::distribute<fluids::grid::device::block_size>(fluids::grid::device::cell_count(grid.grid)), curl_reverse_kernel, grid, collider_ids, field::VectorView<const double>{scratch.vorticity.x, scratch.vorticity.y, scratch.vorticity.z}, scratch.centered_velocity);
            ::cuda::launch(stream, ::cuda::distribute<fluids::grid::device::block_size>(fluids::grid::device::cell_count(grid.grid)), centered_velocity_reverse_kernel, grid, collider_ids, field::VectorView<const double>{scratch.centered_velocity.x, scratch.centered_velocity.y, scratch.centered_velocity.z}, velocity_adjoint);
        }
    } // namespace

    void buoyancy_forward(const ::cuda::stream_ref stream, const device::Discretization grid, const std::uint32_t* collider_ids, const field::ScalarView<const float> density, const field::ScalarView<const float> temperature, const field::VectorView<const float> external_acceleration, const float* ambient_temperature, const float* density_buoyancy, const float* temperature_buoyancy, const field::VectorView<float> force) {
    ...
    void density_buoyancy_forward(const ::cuda::stream_ref stream, const device::Discretization grid, const std::uint32_t* collider_ids, const float buoyancy, const field::ScalarView<const float> density, const field::VectorView<float> force) {
    ...
    void density_buoyancy_vjp(const ::cuda::stream_ref stream, const device::Discretization grid, const std::uint32_t* collider_ids, const float buoyancy, const field::VectorView<const double> force_adjoint, const field::ScalarView<double> density_adjoint) {
    ...
    void buoyancy_jvp(const ::cuda::stream_ref stream, const device::Discretization grid, const std::uint32_t* collider_ids, const field::ScalarView<const float> density, const field::ScalarView<const float> temperature, const field::ScalarView<const float> density_tangent, const field::ScalarView<const float> temperature_tangent, const field::VectorView<const float> external_acceleration_tangent, const float* ambient_temperature, const float* density_buoyancy, const float* temperature_buoyancy, const float* ambient_temperature_tangent, const float* density_buoyancy_tangent, const float* temperature_buoyancy_tangent, const field::VectorView<float> force_tangent) {
    ...
    void buoyancy_vjp(const ::cuda::stream_ref stream, const device::Discretization grid, const std::uint32_t* collider_ids, const field::ScalarView<const float> density, const field::ScalarView<const float> temperature, const float* ambient_temperature, const float* density_buoyancy, const float* temperature_buoyancy, const field::VectorView<const double> force_adjoint, const field::ScalarView<double> density_adjoint, const field::ScalarView<double> temperature_adjoint, const field::VectorView<double> external_acceleration_adjoint, double* ambient_temperature_adjoint, double* density_buoyancy_adjoint, double* temperature_buoyancy_adjoint) {
    ...
    void vorticity_forward(const ::cuda::stream_ref stream, const device::Discretization grid, const std::uint32_t* collider_ids, const field::VectorView<const float> velocity, const float* confinement, const VorticityView cache, const field::VectorView<float> force) {
        vorticity_forward_fields(stream, grid, collider_ids, velocity, cache);
        ::cuda::launch(stream, ::cuda::distribute<fluids::grid::device::block_size>(fluids::grid::device::cell_count(grid.grid)), add_vorticity_force_kernel<DynamicParameter>, grid, collider_ids, field::VectorView<const float>{cache.vorticity.x, cache.vorticity.y, cache.vorticity.z}, field::VectorView<const float>{cache.normal.x, cache.normal.y, cache.normal.z}, DynamicParameter{confinement}, force);
    }

    void vorticity_forward(const ::cuda::stream_ref stream, const device::Discretization grid, const std::uint32_t* collider_ids, const field::VectorView<const float> velocity, const float confinement, const VorticityView cache, const field::VectorView<float> force) {
        vorticity_forward_fields(stream, grid, collider_ids, velocity, cache);
        ::cuda::launch(stream, ::cuda::distribute<fluids::grid::device::block_size>(fluids::grid::device::cell_count(grid.grid)), add_vorticity_force_kernel<ConstantParameter>, grid, collider_ids, field::VectorView<const float>{cache.vorticity.x, cache.vorticity.y, cache.vorticity.z}, field::VectorView<const float>{cache.normal.x, cache.normal.y, cache.normal.z}, ConstantParameter{confinement}, force);
    }

    void vorticity_jvp(const ::cuda::stream_ref stream, const device::Discretization grid, const std::uint32_t* collider_ids, const field::VectorView<const float> velocity_tangent, const float* confinement, const float* confinement_tangent, const ConstVorticityView cache, const field::VectorView<float> force_tangent, const VorticityTangentScratch tangent_scratch) {
        vorticity_tangent_fields(stream, grid, collider_ids, velocity_tangent, cache, tangent_scratch);
        ::cuda::launch(stream, ::cuda::distribute<fluids::grid::device::block_size>(fluids::grid::device::cell_count(grid.grid)), vorticity_force_tangent_kernel<DynamicParameter, DynamicParameter>, grid, collider_ids, cache.vorticity, field::VectorView<const float>{tangent_scratch.vorticity.x, tangent_scratch.vorticity.y, tangent_scratch.vorticity.z}, cache.normal, field::VectorView<const float>{tangent_scratch.normal.x, tangent_scratch.normal.y, tangent_scratch.normal.z}, DynamicParameter{confinement}, DynamicParameter{confinement_tangent}, force_tangent);
    }

    void vorticity_jvp(const ::cuda::stream_ref stream, const device::Discretization grid, const std::uint32_t* collider_ids, const field::VectorView<const float> velocity_tangent, const float confinement, const ConstVorticityView cache, const field::VectorView<float> force_tangent, const VorticityTangentScratch tangent_scratch) {
        vorticity_tangent_fields(stream, grid, collider_ids, velocity_tangent, cache, tangent_scratch);
    ...
    void vorticity_vjp(const ::cuda::stream_ref stream, const device::Discretization grid, const std::uint32_t* collider_ids, const float* confinement, const ConstVorticityView cache, const field::VectorView<const double> force_adjoint, const field::VectorView<double> velocity_adjoint, double* confinement_adjoint, const VorticityAdjointScratch scratch) {
    ...
    void vorticity_vjp(const ::cuda::stream_ref stream, const device::Discretization grid, const std::uint32_t* collider_ids, const float confinement, const ConstVorticityView cache, const field::VectorView<const double> force_adjoint, const field::VectorView<double> velocity_adjoint, const VorticityAdjointScratch scratch) {
        clear_vorticity_adjoint(stream, grid, scratch);
    ...
    void combine_forward(const ::cuda::stream_ref stream, const device::Discretization grid, const std::uint32_t* collider_ids, const field::VectorView<const float> physical, const field::VectorView<const float> control, const field::VectorView<float> total) {
    ...
    void combine_vjp(const ::cuda::stream_ref stream, const device::Discretization grid, const std::uint32_t* collider_ids, const field::VectorView<const double> total_adjoint, const field::VectorView<double> physical_adjoint, const field::VectorView<double> control_adjoint) {
