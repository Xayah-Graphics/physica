...
namespace physica::fluids::grid::device {
    ...
    __host__ __device__ inline Value* component(const field::VectorView<Value> field, const int axis) {
    ...
    __device__ inline Vector3<float> sample_velocity(const Grid grid, const Vector3<float> position, const field::VectorView<const float> velocity) {
