...
export namespace physica::fluids::liquid::operators {
    ...
    struct UniformGridNeighborhood final {
        ...
        [[nodiscard]] Neighborhood allocate_cache(const Domain& domain) const;
        [[nodiscard]] Workspace allocate_workspace(const Domain& domain) const;
        void build(const Domain& domain, std::uint64_t step_index, const VectorField<float>& positions, Neighborhood& neighborhood, Workspace& workspace) const;
    };
