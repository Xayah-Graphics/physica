#ifndef PHYSICA_FLUIDS_LIQUID_PARTICLE_SPH_IISPH_H
#define PHYSICA_FLUIDS_LIQUID_PARTICLE_SPH_IISPH_H

#include "kernels.h"

#include <cstdint>

namespace physica::fluids::liquid::particle::cuda_detail::iisph {
    ...
    void launch_jacobi_update_forward(::cuda::stream_ref stream, std::uint32_t particle_count, float time_step, float reference_gradient_norm, ParticleParameterView particles, const float* densities, const float* previous_pressures, const float* predicted_densities, const float* jacobi_relaxation, float* pressures);
    void launch_jacobi_update_jvp(::cuda::stream_ref stream, std::uint32_t particle_count, float time_step, float reference_gradient_norm, ParticleParameterView particles, ParticleParameterTangentView particle_tangent, const float* densities, const float* density_tangent, const float* previous_pressures, const float* predicted_densities, const float* jacobi_relaxation, const float* previous_pressure_tangent, const float* predicted_density_tangent, const float* jacobi_relaxation_tangent, float* pressure_tangent);
    void launch_jacobi_update_vjp(::cuda::stream_ref stream, std::uint32_t particle_count, float time_step, float reference_gradient_norm, ParticleParameterView particles, const float* densities, const float* previous_pressures, const float* predicted_densities, const float* jacobi_relaxation, const double* pressure_adjoint, ParticleParameterAdjointView particle_adjoint, double* density_adjoint, double* previous_pressure_adjoint, double* predicted_density_adjoint, double* jacobi_relaxation_adjoint);
