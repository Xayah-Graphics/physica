...
namespace physica::fluids::liquid::particle {
    NeighborhoodSearch::NeighborhoodSearch(const Domain& source_domain)
        : domain(source_domain),
          unsorted_keys(domain.stream, ::cuda::device_default_memory_pool(domain.stream.device()), domain.configuration.particle_count, ::cuda::no_init),
          unsorted_particle_indices(domain.stream, ::cuda::device_default_memory_pool(domain.stream.device()), domain.configuration.particle_count, ::cuda::no_init),
          unsorted_boundary_keys(domain.stream, ::cuda::device_default_memory_pool(domain.stream.device()), domain.configuration.boundary_particles.size(), ::cuda::no_init),
          unsorted_boundary_indices(domain.stream, ::cuda::device_default_memory_pool(domain.stream.device()), domain.configuration.boundary_particles.size(), ::cuda::no_init),
          sort_scratch(domain.stream, ::cuda::device_default_memory_pool(domain.stream.device()), cuda_detail::radix_sort_storage_size(domain.configuration.particle_count), ::cuda::no_init),
    ...
    void NeighborhoodSearch::build(const std::uint64_t step_index, const VectorField& positions, Neighborhood& neighborhood) {
        const DomainConfiguration& configuration = domain.configuration;
        cuda_detail::build_neighborhood(
            domain.stream,
            configuration.particle_count,
            static_cast<std::uint32_t>(configuration.boundary_particles.size()),
            configuration.support_radius,
            ...
            cuda_detail::vector(positions),
            cuda_detail::vector(domain.boundary.positions),
            cuda_detail::vector(domain.boundary.velocities),
            unsorted_keys.data(),
            unsorted_particle_indices.data(),
            unsorted_boundary_keys.data(),
            unsorted_boundary_indices.data(),
            sort_scratch.data(),
            sort_scratch.size(),
            boundary_sort_scratch.data(),
            boundary_sort_scratch.size(),
            neighborhood.sorted_keys.data(),
            neighborhood.sorted_particle_indices.data(),
            neighborhood.cell_offsets.data(),
            neighborhood.sorted_boundary_keys.data(),
            neighborhood.sorted_boundary_indices.data(),
            neighborhood.boundary_cell_offsets.data()
        );
