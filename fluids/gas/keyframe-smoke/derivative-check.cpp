...
module physica.fluids.gas.keyframe_smoke.derivative_check;
...
namespace physica::fluids::gas::keyframe_smoke {
    ...
    DirectionalDerivativeCheck DerivativeChecker::directional(const std::span<const double> parameters, const std::span<const double> direction, const double epsilon) {
        ...
        const double finite_difference_jvp_scale = std::max({1.0, std::abs(finite_difference), std::abs(analytic.summary.directional_derivative)});
        const double jvp_vjp_scale = std::max({1.0, std::abs(analytic.summary.directional_derivative), std::abs(analytic.summary.gradient_dot_direction)});
        return {
    ...
    std::vector<ComponentDerivativeCheck> DerivativeChecker::components(const std::span<const double> parameters, const std::span<const std::size_t> parameter_indices, const double epsilon) {
        ...
        for (const std::size_t parameter : parameter_indices) {
            ...
            result.push_back({
                ...
                .analytic = gradient,
                .finite_difference = finite_difference,
                .relative_error = std::abs(gradient - finite_difference) / std::max({1.0, std::abs(gradient), std::abs(finite_difference)}),
            });
        }
        return result;
    }
