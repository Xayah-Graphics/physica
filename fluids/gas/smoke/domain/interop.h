#ifndef PHYSICA_FLUIDS_GAS_SMOKE_DOMAIN_INTEROP_H
#define PHYSICA_FLUIDS_GAS_SMOKE_DOMAIN_INTEROP_H
...
namespace physica::fluids::gas::smoke::cuda_detail {
    ...
    ConstScalarView scalar(const Field& field) {
        return {.values = field.values.data()};
    }

    template<class Field>
    ScalarView scalar(Field& field) {
        return {.values = field.values.data()};
    }

    template<class Field>
    ConstCenteredVectorView centered(const Field& field) {
        return {.x = field.x.values.data(), .y = field.y.values.data(), .z = field.z.values.data()};
    }

    template<class Field>
    CenteredVectorView centered(Field& field) {
        return {.x = field.x.values.data(), .y = field.y.values.data(), .z = field.z.values.data()};
    }

    template<class Field>
    ConstStaggeredVectorView staggered(const Field& field) {
        return {.x = field.x.data(), .y = field.y.data(), .z = field.z.data()};
    }

    template<class Field>
    StaggeredVectorView staggered(Field& field) {
        return {.x = field.x.data(), .y = field.y.data(), .z = field.z.data()};
    }

    template<class Field>
    ConstScalarAdjointView scalar_adjoint(const Field& field) {
        return {.values = field.values.data()};
    }

    template<class Field>
    ScalarAdjointView scalar_adjoint(Field& field) {
        return {.values = field.values.data()};
    }

    template<class Field>
    ConstCenteredVectorAdjointView centered_adjoint(const Field& field) {
        return {.x = field.x.values.data(), .y = field.y.values.data(), .z = field.z.values.data()};
    }

    template<class Field>
    CenteredVectorAdjointView centered_adjoint(Field& field) {
        return {.x = field.x.values.data(), .y = field.y.values.data(), .z = field.z.values.data()};
    }

    template<class Field>
    ConstStaggeredVectorAdjointView staggered_adjoint(const Field& field) {
        return {.x = field.x.data(), .y = field.y.data(), .z = field.z.data()};
    }

    template<class Field>
    StaggeredVectorAdjointView staggered_adjoint(Field& field) {
        return {.x = field.x.data(), .y = field.y.data(), .z = field.z.data()};
    }
