...
namespace physica::fluids::gas::adjoint_control::cuda_backend {
    namespace {
        ...
        __global__ void apply_control_kernel(const detail::cuda::Grid grid, const std::uint32_t step, const ControlLatticeData lattice, const float sigma, const std::uint32_t step_count, const double* parameters, const detail::cuda::CenteredVectorView<float> output) {
            ...
            for (int cz = min_z; cz <= max_z; ++cz)
                for (int cy = min_y; cy <= max_y; ++cy)
                    for (int cx = min_x; cx <= max_x; ++cx) {
                        ...
                        const std::uint64_t center = detail::cuda::index3(cx, cy, cz, lattice.x, lattice.y);
        ...
        __global__ void control_vjp_kernel(const detail::cuda::Grid grid, const std::uint32_t step, const ControlLatticeData lattice, const float sigma, const std::uint32_t step_count, const detail::cuda::CenteredVectorView<const double> output_adjoint, double* gradient) {
            ...
            int cx, cy, cz;
            detail::cuda::decode(center, lattice.x, lattice.y, cx, cy, cz);
            const float center_x = (cx + 0.5F) * grid.nx * grid.cell_size / lattice.x;
            const float center_y = (cy + 0.5F) * grid.ny * grid.cell_size / lattice.y;
            const float center_z = (cz + 0.5F) * grid.nz * grid.cell_size / lattice.z;
            const float support  = 3.0F * sigma;
            ...
            for (int z = min_z; z <= max_z; ++z)
                for (int y = min_y; y <= max_y; ++y)
                    for (int x = min_x; x <= max_x; ++x) {
                        ...
                        const std::uint64_t cell = detail::cuda::index3(x, y, z, grid.nx, grid.ny);
    ...
    void control_forward(const ::cuda::stream_ref stream, const detail::cuda::Grid grid, const std::uint32_t step, const ControlLatticeData lattice, const float sigma, const std::uint32_t step_count, const double* parameters, const detail::cuda::CenteredVectorView<float> output) {
        ::cuda::launch(stream, ::cuda::distribute<detail::cuda::block_size>(detail::cuda::cell_count(grid)), apply_control_kernel, grid, step, lattice, sigma, step_count, parameters, output);
    }

    void control_jvp(const ::cuda::stream_ref stream, const detail::cuda::Grid grid, const std::uint32_t step, const ControlLatticeData lattice, const float sigma, const std::uint32_t step_count, const double* direction, const detail::cuda::CenteredVectorView<float> output) {
        ::cuda::launch(stream, ::cuda::distribute<detail::cuda::block_size>(detail::cuda::cell_count(grid)), apply_control_kernel, grid, step, lattice, sigma, step_count, direction, output);
    }

    void control_vjp(const ::cuda::stream_ref stream, const detail::cuda::Grid grid, const std::uint32_t step, const ControlLatticeData lattice, const float sigma, const std::uint32_t step_count, const detail::cuda::CenteredVectorView<const double> output_adjoint, double* gradient) {
        const std::uint64_t centers = static_cast<std::uint64_t>(lattice.x) * lattice.y * lattice.z;
        ::cuda::launch(stream, ::cuda::distribute<detail::cuda::block_size>(centers), control_vjp_kernel, grid, step, lattice, sigma, step_count, output_adjoint, gradient);
    }
} // namespace physica::fluids::gas::adjoint_control::cuda_backend
