...
namespace physica::optimization {
    ...
    Lbfgsb::Lbfgsb(LbfgsbConfiguration next_configuration, const std::span<const double> initial_parameters, const std::span<const double> next_lower_bounds, const std::span<const double> next_upper_bounds) : configuration(std::move(next_configuration)), lower_bounds(next_lower_bounds.begin(), next_lower_bounds.end()), upper_bounds(next_upper_bounds.begin(), next_upper_bounds.end()), parameters(initial_parameters.begin(), initial_parameters.end()), gradient(parameters.size()), projected_gradient(parameters.size()), direction(parameters.size()), base_parameters(parameters.size()), base_gradient(parameters.size()), trial_parameters(parameters.size()) {
    ...
    void Lbfgsb::subspace_minimization(const HessianRepresentation& hessian, const std::span<const double> cauchy, const std::span<double> candidate) const {
        ...
        if (all_free) {
            ...
            for (std::size_t iteration = 0u; iteration <= 2u * corrections.size() && residual_squared > 1.0e-20 * initial_residual_squared; ++iteration) {
                ...
                for (std::size_t parameter = 0u; parameter < parameters.size(); ++parameter)
                    if (free[parameter] == 0u) hessian_conjugate[parameter] = 0.0;
